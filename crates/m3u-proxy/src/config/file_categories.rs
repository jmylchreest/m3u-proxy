...
impl FileManagerConfig {
    ...
    pub fn with_defaults(base_directory: PathBuf) -> Self {
        ...
        // Preview files - very short retention for temporary previews
        categories.insert("preview".to_string(), FileCategoryConfig {
            subdirectory: "previews".to_string(),
        ...
        // Logo cache - long retention for cached images  
        categories.insert("logo".to_string(), FileCategoryConfig {
            subdirectory: "logos".to_string(),
            retention_duration: humantime::parse_duration("3months").unwrap(), // 3 months (exactly)
            time_match: TimeMatch::LastAccess,
            enabled: true,
        ...
        categories.insert("proxy_output".to_string(), FileCategoryConfig {
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert!(config.get_category("preview").is_some());
        assert!(config.get_category("logo").is_some());
        assert!(config.get_category("proxy_output").is_some());
        
        // Check preview category has short retention
        let preview = config.get_category("preview").unwrap();
        assert_eq!(preview.retention_duration, Duration::from_secs(5 * 60));
        assert_eq!(preview.time_match, TimeMatch::LastAccess);
    ...
    fn test_cleanup_frequency_precision() {
        ...
        // Preview files (5 min retention) should check every minute
        // This gives max 1 minute drift (5 min + 1 min = 6 min max)
        let preview_interval = config.cleanup_interval_for_category("preview").unwrap();
        assert_eq!(preview_interval, Duration::from_secs(60));
        
        // Logo files (90 day retention) should check every 12 hours  
        // This gives max 12 hour drift (90 days + 12 hours = 90.5 days max)
        let logo_interval = config.cleanup_interval_for_category("logo").unwrap();
        ...
        let proxy_interval = config.cleanup_interval_for_category("proxy_output").unwrap();
