...
impl ProgressStageUpdater {
    ...
    pub async fn update_progress(&self, stage_percentage: f64, stage_description: &str) {
        self.manager
            .update_stage_progress(&self.stage_id, stage_percentage, stage_description)
            .await;
    }
    ...
    pub async fn update_channel_program_progress(
        ...
        self.update_progress(percentage.clamp(5.0, 95.0), stage_description)
            .await;
    }
...
impl UniversalProgress {
    ...
    pub fn complete_stage(&mut self, stage_id: &str) -> &mut Self {
        ...
        if self
            .stages
            .iter()
            .all(|s| s.state == UniversalState::Completed)
        {
        ...
        if let Some(current_stage) = self.stages.first_mut() {
                ...
                && total_count > 0
            {
                current_stage.percentage =
                    (processed as f64 / total_count as f64 * 100.0).clamp(0.0, 100.0);
            }
            current_stage.stage_step = stage_step;
        }
        self.last_update = Utc::now();
...
impl ProgressManager {
    ...
    async fn recalculate_and_broadcast(&self) {
        ...
        let stage_weight = if stage_count > 0 {
            100.0 / stage_count as f64
        } else {
            0.0
        };
            ...
            .map(|stage| {
                let stage_state = if stage.is_complete() {
                    ...
                    if description.contains("database")
                        || description.contains("Inserting")
                        || description.contains("Saving")
                        || description.contains("programs")
                    {
    ...
    pub fn create_pipeline_callback(&self) -> Arc<UniversalProgressCallback> {
        ...
        Arc::new(Box::new(move |universal_progress: UniversalProgress| {
            ...
            tokio::spawn(async move {
                ...
                if !universal_progress.current_stage.is_empty()
                    && universal_progress.current_stage != "initializing"
                {
                    ...
                    if let Some(stage_updater) = manager.get_stage_updater(stage_id).await {
                        ...
                        debug!(
                            "Stage '{}' does not exist in ProgressManager - skipping update",
                            stage_id
                        );
                    }
                }
...
impl ProgressService {
            ...
            while let Ok(progress) = cleanup_rx.recv().await {
                ...
                if progress.is_complete() {
                    ...
                    if active.remove(&progress.owner_id) {
                        debug!(
                            "Cleaned up active operation for owner {} (state: {:?})",
                            progress.owner_id, progress.state
                        );
                    } else {
                        debug!(
                            "Attempted to clean up already removed operation for owner {} (state: {:?})",
                            progress.owner_id, progress.state
                        );
                    }
                }
    ...
    pub async fn create_staged_progress_manager(
        ...
        if self.is_operation_in_progress(owner_id).await {
            let error_msg = format!(
                "Cannot create staged progress manager for owner {owner_id}: Operation already in progress"
            );
            warn!("{}", error_msg);
    ...
    pub async fn is_operation_in_progress(&self, owner_id: Uuid) -> bool {
        ...
        let active_operations: Vec<_> = storage
            .values()
        ...
        if !active_operations.is_empty() {
            warn!(
                "Found {} active operations for owner {}: {:?}",
                active_operations.len(),
                owner_id,
                active_operations
                    .iter()
                    .map(|p| (p.id, &p.operation_name, &p.state))
                    .collect::<Vec<_>>()
            );
        }
    ...
    pub async fn force_cleanup_owner_operations(&self, owner_id: Uuid) {
        ...
        if !owner_operation_ids.is_empty() {
            warn!(
                "Force cleaning up {} stuck operations for owner {}",
                owner_operation_ids.len(),
                owner_id
            );
    ...
    pub async fn start_operation_with_context(
        ...
        self.start_operation(owner_id, owner_type, operation_type, operation_name)
            .await
    }
            ...
            match progress.operation_type {
                OperationType::StreamIngestion | OperationType::EpgIngestion => {
                    ...
                    for stage in &progress.stages {
                        if matches!(
                            stage.state,
                            UniversalState::Saving | UniversalState::Processing
                        ) && stage.percentage < 100.0
                        {
                            tracing::info!(
    ...
    pub async fn cleanup_stale_operations(&self, max_age_minutes: i64) {
        ...
        for (id, progress) in storage.iter() {
            if !progress.is_complete() && progress.last_update < cutoff_time {
                warn!(
                    "Cleaning up stale operation: {} ({}) - last update was {} minutes ago",
                    id,
                    progress.operation_name,
        ...
        if !storage.is_empty() {
            info!(
                "Cleaned up stale operations, {} active operations remaining",
                storage.len()
            );
        }
    }
    ...
    pub async fn force_clear_owner_operations(&self, owner_id: Uuid) {
        ...
        for (id, progress) in storage.iter() {
            if progress.owner_id == owner_id {
                warn!(
                    "Force clearing operation: {} ({}) for owner {}",
...
mod tests {
    ...
    async fn test_single_stage_progress() {
        ...
        let progress_manager =
            ProgressManager::new(Uuid::new_v4(), "test_operation".to_string(), None, tx, None);
    ...
    async fn test_multi_stage_progress_calculation() {
        ...
        let progress_manager = progress_manager
            .add_stage("stage1", "Stage 1")
            .await
            .add_stage("stage2", "Stage 2")
            .await
            .add_stage("stage3", "Stage 3")
            .await
            .add_stage("stage4", "Stage 4")
            .await;
    ...
    async fn test_update_items_helper() {
        let (tx, _rx) = broadcast::channel(100);
        let progress_manager =
            ProgressManager::new(Uuid::new_v4(), "items_test".to_string(), None, tx, None);

        let progress_manager = progress_manager
            .add_stage("processing", "Processing Items")
            .await;
        let updater = progress_manager
            .get_stage_updater("processing")
            .await
            .unwrap();
        ...
        updater
            .update_items(100, Some(100), "Processing complete")
            .await;
    ...
    async fn test_stage_progression_order() {
        ...
        // Add stages in order
        let progress_manager = progress_manager
            .add_stage("data_mapping", "Data Mapping")
            .await
            .add_stage("filtering", "Filtering")
            .await
            .add_stage("generation", "Generation")
            .await;
        ...
        let updater1 = progress_manager
            .get_stage_updater("data_mapping")
            .await
            .unwrap();
        updater1.complete_stage().await;
        ...
        let updater2 = progress_manager
            .get_stage_updater("filtering")
            .await
            .unwrap();
        updater2.complete_stage().await;
        ...
        // Complete final stage
        let updater3 = progress_manager
            .get_stage_updater("generation")
            .await
            .unwrap();
        updater3.complete_stage().await;
    ...
    async fn test_concurrent_stage_updates() {
        ...
        let handle1 = {
            ...
            tokio::spawn(async move {
                for i in 0..=10 {
                    updater
                        .update_progress(i as f64 * 10.0, &format!("Stage 1 at {i}"))
                        .await;
        ...
        let handle2 = {
            ...
            tokio::spawn(async move {
                ...
                for i in 0..=10 {
                    updater
                        .update_progress(i as f64 * 10.0, &format!("Stage 2 at {i}"))
                        .await;
                    sleep(Duration::from_millis(1)).await;
                }
                updater.complete_stage().await;
    ...
    async fn test_channel_program_progress_calculation() {
        ...
        let progress_manager = progress_manager
            .add_stage("processing", "Processing Data")
            .await;
        let updater = progress_manager
            .get_stage_updater("processing")
            .await
        ...
        updater
            .complete_preprocessing("Database fetch complete, starting processing")
            .await;
        let progress = progress_manager.get_progress().await;
        ...
        updater
            .update_channel_program_progress(150, 75, 1000, 500, "Processing 15%")
            .await;
        let progress = progress_manager.get_progress().await;
        ...
        updater
            .update_channel_program_progress(500, 250, 1000, 500, "Processing 50%")
            .await;
        let progress = progress_manager.get_progress().await;
        ...
        updater
            .update_channel_program_progress(1000, 500, 1000, 500, "Processing complete")
            .await;
        let progress = progress_manager.get_progress().await;
    ...
    async fn test_preprocessing_workflow() {
        ...
        updater
            .complete_preprocessing("Loaded 1000 channels and 500 programs from database")
            .await;
        let progress = progress_manager.get_progress().await;
        assert_eq!(progress.stages[0].percentage, 5.0);
        ...
        updater
            .update_channel_program_progress(500, 250, 1000, 500, "Mapping 50% complete")
            .await;
        let progress = progress_manager.get_progress().await;
        assert!((progress.stages[0].percentage - 50.0).abs() < 0.01);
    ...
    async fn test_edge_cases_channel_program_progress() {
        ...
        updater
            .update_channel_program_progress(0, 0, 0, 0, "No data to process")
            .await;
        let progress = progress_manager.get_progress().await;
        assert_eq!(progress.stages[0].percentage, 5.0);
        ...
        updater
            .update_channel_program_progress(50, 0, 100, 0, "Channels only")
            .await;
        let progress = progress_manager.get_progress().await;
        ...
        updater
            .update_channel_program_progress(0, 25, 0, 100, "Programs only")
            .await;
        let progress = progress_manager.get_progress().await;
        ...
        updater
            .update_channel_program_progress(1000, 1000, 1000, 1000, "At boundary")
            .await;
        let progress = progress_manager.get_progress().await;
        assert_eq!(progress.stages[0].percentage, 95.0);
    }
