...
impl SimpleMemoryMonitor {
    ...
    pub fn initialize(&mut self) -> Result<()> {
        ...
        info!("Memory monitor initialized: baseline {:.1}MB", rss as f64 / 1024.0 / 1024.0);
        Ok(())
    }
    ...
    pub fn observe_stage(&mut self, stage_name: &str) -> Result<MemorySnapshot> {
        ...
        self.stage_memories.insert(stage_name.to_string(), current_rss);

        // Check for memory warnings
        self.check_memory_warning(&snapshot);

        debug!("Memory observation for '{}': {:.1}MB (Δ{:+.1}MB)", 
    ...
    pub fn check_memory_limit(&self) -> Result<MemoryLimitStatus> {
        ...
        if let Some(limit_mb) = self.memory_limit_mb {
            ...
            if usage_ratio >= 1.0 {
                warn!("Memory limit exceeded: {:.1}MB > {}MB", current_mb, limit_mb);
                return Ok(MemoryLimitStatus::Exceeded);
            } else if usage_ratio >= self.warning_threshold {
                warn!("Memory usage near limit: {:.1}% ({:.1}MB / {}MB)", 
                      usage_ratio * 100.0, current_mb, limit_mb);
    ...
    pub fn get_statistics(&self) -> MemoryStats {
        ...
        let within_limits = self.memory_limit_mb
    ...
    fn get_current_rss(&self) -> Result<u64> {
        #[cfg(target_os = "linux")]
        {
    ...
    /// Check for memory warnings
    fn check_memory_warning(&self, snapshot: &MemorySnapshot) {
        if let Some(limit_mb) = self.memory_limit_mb {
            let usage_ratio = snapshot.rss_mb / limit_mb as f64;
            
            if usage_ratio >= self.warning_threshold {
                warn!(
                    "Memory usage warning in stage '{}': {:.1}% of limit ({:.1}MB / {}MB)",
                    snapshot.stage,
                    usage_ratio * 100.0,
                    snapshot.rss_mb,
                    limit_mb
...
impl MemoryLimitStatus {
    ...
    pub fn get_suggestions(&self) -> &'static str {
        match self {
            ...
            MemoryLimitStatus::Warning => "Consider: reducing batch sizes, processing in chunks, or freeing unused data",
            MemoryLimitStatus::Exceeded => "Immediate action required: stop current processing, free memory, or increase limits",
        }
    }
