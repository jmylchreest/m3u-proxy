...
pub async fn health_check(
    ...
    log_request(&axum::http::Method::GET, &"/health".parse().unwrap(), &context);
...
pub async fn detailed_health_check(
    ...
    log_request(&axum::http::Method::GET, &"/health/detailed".parse().unwrap(), &context);

    let mut health_details = std::collections::HashMap::new();
    
    // Database health
    let db_health = check_database_health(&state.database).await;
    health_details.insert("database".to_string(), serde_json::to_value(&db_health).unwrap_or_default());
    
    // Ingestion state manager health
    health_details.insert("ingestion_manager".to_string(), serde_json::json!({
        "status": "active",
        "active_operations": 0  // Would get from actual state manager
    }));
    
    // Cache health
    health_details.insert("cache".to_string(), serde_json::json!({
        "status": "active"
    }));
...
pub async fn readiness_check(
    ...
    log_request(&axum::http::Method::GET, &"/ready".parse().unwrap(), &context);
...
pub async fn liveness_check(
    ...
    log_request(&axum::http::Method::GET, &"/live".parse().unwrap(), &_context);
