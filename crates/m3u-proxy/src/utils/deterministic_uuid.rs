...
pub fn generate_channel_uuid(source_id: &Uuid, stream_url: &str, channel_name: &str) -> Uuid {
    generate_deterministic_uuid(&[&source_id.to_string(), &stream_url, &channel_name])
}
...
pub fn generate_proxy_config_uuid(proxy_id: &Uuid, config_identifier: &str) -> Uuid {
    generate_deterministic_uuid(&[&proxy_id.to_string(), &config_identifier])
}
...
mod tests {
    ...
    fn test_different_inputs_different_uuids() {
        ...
        let uuid1 =
            generate_channel_uuid(&source_id, "http://example.com/stream1.m3u8", "Channel 1");
        let uuid2 =
            generate_channel_uuid(&source_id, "http://example.com/stream2.m3u8", "Channel 2");
