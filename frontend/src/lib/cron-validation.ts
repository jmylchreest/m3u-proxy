...
export function validateCronExpression(cronExpression: string): CronValidationResult {
  ...
  if (fields.length !== 7) {
    ...
    if (fields.length === 5) {
      ...
      suggestion = 'Convert from 5-field Unix format to 7-field format by adding seconds and year: "0 ' + trimmed + ' *"'
  ...
  const fieldNames = ['seconds', 'minutes', 'hours', 'day-of-month', 'month', 'day-of-week', 'year']
  const fieldRanges = [
    [0, 59],   // seconds
    [0, 59],   // minutes
    [0, 23],   // hours
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if (field === '*' || field === '?' || field.includes('/') || field.includes('-') || field.includes(',')) {
