...
use crate::utils::{CircuitBreaker, CompressionFormat, DecompressionService};
...
impl StandardHttpClient {
    ...
    pub async fn fetch_logo(&self, logo_url: &str) -> Result<bytes::Bytes, String> {
        debug!(
            "Attempting to fetch logo: {}",
            UrlUtils::obfuscate_credentials(logo_url)
        );
        ...
        let request_fn = || async {
            ...
            if !crate::utils::is_status_acceptable(
                &response.status(),
        ...
        if let Some(circuit_breaker) = &self.circuit_breaker {
            ...
            match cb_result.result {
                Ok(bytes) => {
                    debug!(
                        "Successfully fetched logo: {} ({} bytes, took {:?}, CB state: {:?})",
                        UrlUtils::obfuscate_credentials(logo_url),
                        bytes.len(),
                        cb_result.execution_time,
                        cb_result.state
                    );
                    Ok(bytes)
                }
                Err(crate::utils::circuit_breaker::CircuitBreakerError::CircuitOpen) => {
                    debug!(
                        "Logo fetch blocked by circuit breaker: {} (state: {:?})",
                        UrlUtils::obfuscate_credentials(logo_url),
                        cb_result.state
                    );
                ...
                Err(crate::utils::circuit_breaker::CircuitBreakerError::ServiceError(e)) => {
                    debug!(
                        "Logo fetch failed: {} - {} (CB state: {:?}, took {:?})",
                        UrlUtils::obfuscate_credentials(logo_url),
                        e,
                        cb_result.state,
                        cb_result.execution_time
                ...
                Err(crate::utils::circuit_breaker::CircuitBreakerError::Timeout) => {
                    debug!(
                        "Logo fetch timed out: {} (CB state: {:?}, took {:?})",
                        UrlUtils::obfuscate_credentials(logo_url),
                        cb_result.state,
                        cb_result.execution_time
                    );
    ...
    async fn process_response_to_bytes(response: Response, url: &str) -> AppResult<Vec<u8>> {
        ...
        let decompressed_bytes = match compression_format {
            ...
            _ => {
                ...
                DecompressionService::decompress(bytes).map_err(|e| {
                    AppError::source_error(format!("Failed to decompress content: {e}"))
                })?
            }
        };

        debug!(
            "Successfully processed {} bytes of content (compression: {:?})",
...
impl DecompressingHttpClient for StandardHttpClient {
    async fn fetch_text(&self, url: &str) -> AppResult<String> {
        ...
        let response = if let Some(circuit_breaker) = &self.circuit_breaker {
            ...
            // No circuit breaker - direct request
            request_fn().await.map_err(|e| AppError::ExternalService {
                service: "http_client".to_string(),
                message: e,
            })?
        };
        ...
        let content = String::from_utf8(decompressed_bytes).map_err(|e| {
            AppError::source_error(format!("Failed to decode content as UTF-8: {e}"))
        })?;
    ...
    async fn fetch_json<T: DeserializeOwned + Send>(&self, url: &str) -> AppResult<T> {
        ...
        let response = if let Some(circuit_breaker) = &self.circuit_breaker {
            ...
            match cb_result.result {
                Ok(response) => {
                    debug!(
                        "JSON request successful: {} (CB state: {:?}, took {:?})",
                        UrlUtils::obfuscate_credentials(url),
                        cb_result.state,
                        cb_result.execution_time
                    );
    ...
    async fn fetch_bytes(&self, url: &str) -> AppResult<Vec<u8>> {
        ...
        let response = if let Some(circuit_breaker) = &self.circuit_breaker {
            ...
            match cb_result.result {
                Ok(response) => {
                    debug!(
                        "Binary request successful: {} (CB state: {:?}, took {:?})",
                        UrlUtils::obfuscate_credentials(url),
                        cb_result.state,
                        cb_result.execution_time
                    );
        ...
        debug!(
            "Successfully fetched {} bytes of binary content",
            decompressed_bytes.len()
        );
        Ok(decompressed_bytes)
    }
    ...
    async fn fetch_text_with_headers(
        ...
        debug!(
            "Fetching text content with headers from: {}",
            UrlUtils::obfuscate_credentials(url)
        );
        ...
        let response = if let Some(circuit_breaker) = &self.circuit_breaker {
            ...
            match cb_result.result {
                Ok(response) => {
                    debug!(
                        "Text with headers request successful: {} (CB state: {:?}, took {:?})",
                        UrlUtils::obfuscate_credentials(url),
                        cb_result.state,
                        cb_result.execution_time
                    );
        ...
        debug!(
            "Successfully fetched {} characters of text content with headers",
            content.len()
        );
        Ok(content)
    }
