...
impl crate::database::Database {
    ...
    pub async fn list_linked_xtream_sources(&self) -> Result<Vec<LinkedXtreamSources>> {
        ...
        for row in rows {
            ...
            linked_sources.push(LinkedXtreamSources {
                ...
                created_at: crate::utils::datetime::DateTimeParser::parse_flexible(&created_at).map_err(|_| anyhow::anyhow!("Failed to parse created_at"))?,
                updated_at: crate::utils::datetime::DateTimeParser::parse_flexible(&updated_at).map_err(|_| anyhow::anyhow!("Failed to parse updated_at"))?,
            });
        }
    ...
    pub async fn get_linked_xtream_source(
        ...
        if let Some(row) = row {
            ...
            Ok(Some(LinkedXtreamSources {
                ...
                created_at: crate::utils::datetime::DateTimeParser::parse_flexible(&row.try_get::<String, _>("created_at")?).map_err(|_| anyhow::anyhow!("Failed to parse created_at"))?,
                updated_at: crate::utils::datetime::DateTimeParser::parse_flexible(&row.try_get::<String, _>("updated_at")?).map_err(|_| anyhow::anyhow!("Failed to parse updated_at"))?,
            }))
        } else {
            Ok(None)
        }
    ...
    pub async fn find_linked_epg_by_stream_id(
        ...
        if let Some(row) = row {
            ...
            let created_at = crate::utils::datetime::DateTimeParser::parse_flexible(&row.get::<String, _>("created_at")).map_err(|_| anyhow::anyhow!("Failed to parse created_at"))?;
            let updated_at = crate::utils::datetime::DateTimeParser::parse_flexible(&row.get::<String, _>("updated_at")).map_err(|_| anyhow::anyhow!("Failed to parse updated_at"))?;
            let last_ingested_at = row
                .get::<Option<String>, _>("last_ingested_at")
                .map(|s| crate::utils::datetime::DateTimeParser::parse_flexible(&s).map_err(|_| anyhow::anyhow!("Failed to parse datetime")))
                .transpose()?;
