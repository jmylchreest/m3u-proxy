...
use crate::web::handlers::health::{check_ffmpeg_availability, check_ffprobe_availability, check_hardware_acceleration};
...
pub async fn list_profiles(State(state): State<AppState>) -> impl IntoResponse {
    let relay_repo = crate::database::repositories::RelaySeaOrmRepository::new(state.database.connection().clone());
...
pub async fn get_relay_health(State(state): State<AppState>) -> impl IntoResponse {
    ...
    match state.relay_manager.get_relay_health().await {
        Ok(relay_health) => {
            ...
            let processes: Vec<crate::web::responses::RelayProcess> = relay_health.processes.into_iter()
                .map(|p| crate::web::responses::RelayProcess {
                    ...
                    connected_clients: p.connected_clients.into_iter()
            ...
            let dashboard_response = crate::web::responses::RelayHealthApiResponse {
                status: if relay_health.unhealthy_processes == 0 { "healthy".to_string() } else { "degraded".to_string() },
