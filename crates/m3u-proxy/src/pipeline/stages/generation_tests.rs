...
impl XmltvTestUtils {
    ...
    pub fn validate_xmltv_structure(content: &str) -> Result<XmltvValidationResult, Box<dyn std::error::Error>> {
                        ...
                        "tv" => {
                            ...
                            for attr in e.attributes() {
                                ...
                                match key {
                                    ...
                                    "source-info-name" => validation_result.has_source_info_name = true,
                                    "source-info-url" => validation_result.has_source_info_url = true,
                                    "generator-info-name" => validation_result.has_generator_info_name = true,
                                    "generator-info-url" => validation_result.has_generator_info_url = true,
                                    _ => {}
                                }
                                
                                validation_result.tv_attributes.insert(key.to_string(), value.to_string());
                            }
                        },
                        "channel" => {
                            if in_tv_root {
                        ...
                        "programme" => {
                            if in_tv_root {
                                ...
                                for attr in e.attributes() {
                                    ...
                                    match key {
                                        "channel" => programme_info.channel_id = Some(value.to_string()),
                                        "start" => programme_info.start_time = Some(value.to_string()),
                ...
                Ok(Event::Text(ref e)) => {
                    ...
                    if !text.trim().is_empty() && !current_element_stack.is_empty() {
                        ...
                        if current_element == "title" && current_element_stack.len() >= 2 {
                            ...
                            if parent == "programme"
                                && let Some(last_programme) = validation_result.programmes.last_mut() {
...
impl XmltvValidationResult {
    ...
    pub fn validate(&self) -> bool {
        self.has_tv_root 
            && self.has_date_attribute 
            && self.has_source_info_name
...
<tv date="01/01/2024 12:00:00" source-info-url="https://github.com/jmylchreest/m3u-proxy" source-info-name="m3u-proxy" generator-info-name="m3u-proxy" generator-info-url="https://github.com/jmylchreest/m3u-proxy">
  ...
  <programme channel="cnn-intl" start="20240101120000 +0000" stop="20240101130000 +0000">
    ...
    fn test_xmltv_structure_validation() {
        ...
        assert!(validation_result.has_tv_root, "Should have <tv> root element");
        assert!(validation_result.has_date_attribute, "Should have date attribute");
        assert!(validation_result.has_source_info_name, "Should have source-info-name");
        assert!(validation_result.has_generator_info_name, "Should have generator-info-name");

        // Validate channels
        assert_eq!(validation_result.channels_count, 2, "Should have 2 channels");
        assert!(validation_result.channel_ids.contains(&"bbc-one-hd".to_string()));
        assert!(validation_result.channel_ids.contains(&"cnn-intl".to_string()));

        // Validate programmes
        assert_eq!(validation_result.programmes_count, 2, "Should have 2 programmes");
        
        let bbc_programme = validation_result.programmes.iter()
...
<tv>
  <channel id="test">
    <display-name>Test</display-name>
        ...
        assert!(!validation_result.is_valid_xmltv, "Should be invalid without required attributes");
        assert!(!validation_result.has_date_attribute, "Should be missing date attribute");
        assert!(!validation_result.has_source_info_name, "Should be missing source-info-name");
    }
...
<tv date="01/01/2024 12:00:00" source-info-name="m3u-proxy" generator-info-name="m3u-proxy">
  ...
  <programme channel="test-channel" start="20240101120000 +0000" stop="20240101130000 +0000">
    ...
    <category>News &amp; Current Affairs</category>
        ...
        assert!(validation_result.is_valid_xmltv, "Should be valid despite escaped characters");
...
<tv date="01/01/2024 12:00:00" source-info-name="m3u-proxy" generator-info-name="m3u-proxy">
  <channel id="empty-channel">
    <display-name>Empty Channel</display-name>
        ...
        assert!(validation_result.is_valid_xmltv, "Should be valid with no programmes");
        assert_eq!(validation_result.channels_count, 1);
...
<tv date="01/01/2024 12:00:00" source-info-name="m3u-proxy" generator-info-name="m3u-proxy">
  ...
  <programme channel="attrs-test" start="20240101180000 +0000" stop="20240101190000 +0000">
    <title>Test Programme</title>
        ...
        assert_eq!(programme.start_time, Some("20240101180000 +0000".to_string()));
