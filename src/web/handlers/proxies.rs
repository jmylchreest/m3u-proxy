...
impl CreateStreamProxyRequest {
    ...
    pub fn into_service_request(self) -> Result<crate::models::StreamProxyCreateRequest, String> {
        ...
        Ok(crate::models::StreamProxyCreateRequest {
            ...
            stream_sources: self.stream_sources.into_iter().map(|s| crate::models::ProxySourceCreateRequest {
                source_id: s.source_id,
                priority_order: s.priority_order,
            }).collect(),
            epg_sources: self.epg_sources.into_iter().map(|e| crate::models::ProxyEpgSourceCreateRequest {
                epg_source_id: e.epg_source_id,
                priority_order: e.priority_order,
            }).collect(),
            filters: self.filters.into_iter().map(|f| crate::models::ProxyFilterCreateRequest {
...
pub async fn list_proxies(
    ...
    log_request(&axum::http::Method::GET, &"/api/v1/proxies".parse().unwrap(), &context);
...
pub async fn get_proxy(
    ...
    log_request(&axum::http::Method::GET, &format!("/api/v1/proxies/{}", id).parse().unwrap(), &context);
...
pub async fn create_proxy(
    ...
    log_request(&axum::http::Method::POST, &"/api/v1/proxies".parse().unwrap(), &context);
...
pub async fn update_proxy(
    ...
    log_request(&axum::http::Method::PUT, &format!("/api/v1/proxies/{}", id).parse().unwrap(), &context);
    ...
    let service_request = crate::models::StreamProxyUpdateRequest {
        ...
        proxy_mode: match request.proxy_mode.to_lowercase().as_str() {
            ...
            _ => return crate::web::responses::bad_request(&format!("Invalid proxy mode: {}", request.proxy_mode)).into_response(),
        },
        ...
        stream_sources: request.stream_sources.into_iter().map(|s| crate::models::ProxySourceCreateRequest {
            source_id: s.source_id,
            priority_order: s.priority_order,
        }).collect(),
        epg_sources: request.epg_sources.into_iter().map(|e| crate::models::ProxyEpgSourceCreateRequest {
            epg_source_id: e.epg_source_id,
            priority_order: e.priority_order,
        }).collect(),
        filters: request.filters.into_iter().map(|f| crate::models::ProxyFilterCreateRequest {
            filter_id: f.filter_id,
            priority_order: f.priority_order,
            is_active: f.is_active,
        }).collect(),
...
pub async fn delete_proxy(
    ...
    log_request(&axum::http::Method::DELETE, &format!("/api/v1/proxies/{}", id).parse().unwrap(), &context);
    ...
    match service.delete(uuid).await {
        Ok(()) => crate::web::responses::ok(serde_json::json!({"message": "Proxy deleted successfully"})).into_response(),
        Err(err) => crate::web::responses::handle_error(err).into_response(),
    }
}
...
pub async fn preview_proxy_config(
    ...
    log_request(&axum::http::Method::POST, &"/api/v1/proxies/preview".parse().unwrap(), &context);
...
pub async fn preview_existing_proxy(
    ...
    log_request(&axum::http::Method::GET, &format!("/api/v1/proxies/{}/preview", id).parse().unwrap(), &context);
    ...
    match service.get_by_id(uuid).await {
        Ok(Some(proxy_response)) => {
            ...
            let preview_request = PreviewProxyRequest {
                ...
                stream_sources: proxy_response.stream_sources.into_iter().map(|s| ProxySourceRequest {
                    source_id: s.source_id,
                    priority_order: s.priority_order,
                }).collect(),
                epg_sources: proxy_response.epg_sources.into_iter().map(|e| ProxyEpgSourceRequest {
                    epg_source_id: e.epg_source_id,
                    priority_order: e.priority_order,
                }).collect(),
                filters: proxy_response.filters.into_iter().map(|f| ProxyFilterRequest {
                    filter_id: f.filter_id,
                    priority_order: f.priority_order,
                    is_active: f.is_active,
                }).collect(),
...
pub async fn serve_proxy_m3u(
    ...
    let _proxy = match state.database.get_proxy_by_ulid(&ulid).await {
        Ok(proxy) => {
            if !proxy.is_active {
                ...
                return (StatusCode::NOT_FOUND, headers, "#EXTM3U\n# Proxy not active\n".to_string());
            }
            proxy
        }
        Err(e) => {
    ...
    match fs::read_to_string(&m3u_file_path).await {
        Ok(content) => {
            info!("Served static M3U8 for proxy {} from {}", ulid, m3u_file_path.display());
        ...
        Err(e) => {
            error!("Failed to read M3U8 file for proxy {} at {}: {}", ulid, m3u_file_path.display(), e);
...
pub async fn serve_proxy_xmltv(
    ...
    let proxy = match state.database.get_proxy_by_ulid(&ulid).await {
        ...
        Err(e) => {
            ...
            let mut headers = HeaderMap::new();
            headers.insert("content-type", "application/xml".parse().unwrap());
            return (StatusCode::NOT_FOUND, headers, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<tv><!-- Proxy not found --></tv>".to_string());
        }
    };
    
    // 2. Try to serve static XMLTV file from disk
    let xmltv_file_path = state.config.storage.m3u_path.join(format!("{}.xmltv", ulid));
    
    match fs::read_to_string(&xmltv_file_path).await {
        Ok(content) => {
            info!("Served static XMLTV for proxy {} from {}", ulid, xmltv_file_path.display());
        ...
        Err(e) => {
            error!("Failed to read XMLTV file for proxy {} at {}: {}", ulid, xmltv_file_path.display(), e);
...
pub async fn proxy_stream(
    ...
    let channel = match state.database.get_channel_for_proxy(&proxy_ulid, channel_id).await {
        Ok(Some(channel)) => channel,
        Ok(None) => {
            warn!("Channel {} not found in proxy {} or proxy not active", channel_id, proxy_ulid);
            return (StatusCode::NOT_FOUND, "Channel not found in this proxy".to_string()).into_response();
        }
        Err(e) => {
            error!("Failed to lookup channel {} in proxy {}: {}", channel_id, proxy_ulid, e);
    ...
    let session = state.metrics_logger.log_stream_start(
        proxy_ulid.clone(),
        channel_id,
        client_ip.to_string(),
        Some(user_agent.to_string()),
        headers.get("referer").and_then(|h| h.to_str().ok()).map(|s| s.to_string()),
    ).await;
...
async fn proxy_http_stream(
    ...
    let client = match reqwest::Client::builder()
        ...
        Err(e) => {
            ...
            return (StatusCode::INTERNAL_SERVER_ERROR, "Failed to create HTTP client".to_string()).into_response();
        }
    };
    ...
    let upstream_response = match client
        ...
        Err(e) => {
            ...
            return (StatusCode::BAD_GATEWAY, "Failed to connect to upstream".to_string()).into_response();
        }
    };
    ...
    tokio::spawn(async move {
        ...
        final_session.finish(&final_state.metrics_logger, bytes_served).await;
    });
    ...
    match response.body(body) {
        ...
        Err(e) => {
            ...
            (StatusCode::INTERNAL_SERVER_ERROR, "Failed to build response".to_string()).into_response()
        }
    }
}