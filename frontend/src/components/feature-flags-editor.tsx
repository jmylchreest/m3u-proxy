...
export function FeatureFlagsEditor({ 
  ...
  const validateFlagKey = (key: string): string | null => {
    ...
    if (!/^[a-z0-9-]+$/.test(key)) return "Flag key must contain only lowercase letters, numbers, and hyphens"
    if (key.startsWith('-') || key.endsWith('-')) return "Flag key cannot start or end with a hyphen"
  ...
  const updateConfigValue = (index: number, field: keyof FeatureConfigValue, value: string) => {
    setConfigValues(prev => prev.map((item, i) => 
  ...
  const validateConfigKey = (key: string): string | null => {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(key)) return "Config key must contain only letters, numbers, underscores, and hyphens"
  ...
  const saveConfig = () => {
    ...
    configValues.forEach(({ key, value, type }, index) => {
      if (key.trim()) {
        ...
        if (type === 'boolean' && value && value.trim() && !['true', 'false'].includes(value.toLowerCase())) {
    ...
    setFlags(prev => prev.map(flag => 
      flag.key === showEditConfig ? { ...flag, config } : flag
    ))
                  ...
                  <TableCell>
                    {Object.keys(flag.config).length > 0 ? (
                      <div className="space-y-1">
                        {Object.entries(flag.config).slice(0, 2).map(([key, value]) => (
