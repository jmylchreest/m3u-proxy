...
impl LogoCacheService {
    ...
    async fn create_entry_from_filesystem(&self, file_name: &str, file_size: u64) -> Result<LogoCacheEntry> {
                ...
                match String::from_utf8(metadata_bytes) {
                    Ok(metadata_content) => {
                        ...
                        match serde_json::from_str::<CachedLogoMetadata>(&metadata_content) {
                            Ok(metadata) => {
                                debug!("Loaded metadata for {}: channel={:?}, group={:?}", 
                    ...
                    Err(e) => {
                        debug!("Failed to convert metadata bytes to string for {}: {}", cache_id, e);
                        let (w, h) = self.parse_dimensions_from_filename(file_name);
                        (None, None, None, w, h)
                    }
                }
    ...
    pub async fn search(&self, query: &LogoCacheQuery) -> Result<Vec<LogoCacheSearchResult>> {
        ...
        if let Some(hash) = url_hash
            ...
            results.push(self.entry_to_search_result(entry, &query.original_url).await);
            return Ok(results);
        }
            ...
            if let Some(url_hashes) = name_index.get(&hash) {
                for &url_hash in url_hashes {
                    if let Some(entry) = cache_index.get(&url_hash) {
                        results.push(self.entry_to_search_result(entry, &query.channel_name).await);
                    }
                }
            }
        }
                ...
                for &url_hash in url_hashes {
                    if let Some(entry) = cache_index.get(&url_hash) {
                        ...
                        if !results.iter().any(|r| r.url_hash == entry.url_hash) {
                            results.push(self.entry_to_search_result(entry, &query.channel_group).await);
                        }
                    }
                }
            }
    ...
    async fn search_substring(&self, query: &LogoCacheQuery) -> Result<Vec<LogoCacheSearchResult>> {
        ...
        for (_, entry) in cache_index.iter() {
            ...
            let metadata_filename = format!("{}.json", 
            ...
            if let Ok(Some(metadata)) = self.read_metadata_file(&metadata_filename).await {
                ...
                if let Some(ref extra_fields) = metadata.extra_fields {
                    for (key, value) in extra_fields {
                        if key.to_lowercase().contains(&search_term) || value.to_lowercase().contains(&search_term) {
        ...
        debug!("Substring search for '{}': {} searched, {} matched, {} results", 
               search_term, searched_count, matched_count, results.len());
    ...
    async fn read_metadata_file(&self, filename: &str) -> Result<Option<crate::services::logo_cache::CachedLogoMetadata>> {
        ...
        // Find the entry by cache_id by scanning the primary index
        {
            ...
            for (url_hash, entry) in cache_index.iter() {
                ...
                if entry_cache_id == cache_id {
                    ...
                    debug!("Found logo cache entry to remove: cache_id={}, url_hash={}", cache_id, url_hash);
                    break;
                }
            }
        }
    ...
    async fn entry_to_search_result(&self, entry: &LogoCacheEntry, matched_string: &Option<String>) -> LogoCacheSearchResult {
        ...
        let matched_text = if let Some(text) = matched_string {
            ...
            if let Some(cached) = string_cache.get(&entry.url_hash) {
                ...
                self.load_channel_name_from_metadata(&entry.relative_path).await
            }
        };
        ...
        let json_path = std::path::Path::new(&entry.relative_path)
            ...
            .to_string_lossy()
            .to_string();
        let (cached_at, updated_at) = if let Ok(Some(metadata)) = self.read_metadata_file(&json_path).await {
    ...
    pub async fn run_maintenance(&self, max_cache_size_mb: u64, max_age_days: u64) -> Result<MaintenanceStats> {
        ...
        for (url_hash, entry) in entries {
            ...
            if should_remove_by_age || should_remove_by_size {
                ...
                match self.logo_file_manager.remove_file(&entry.relative_path).await {
                    Ok(()) => {
                        ...
                        debug!("Removed cached logo: {} (age: {}d)", entry.relative_path, age_seconds / 86400);
                    },
                    Err(e) => {
                        warn!("Failed to remove cached logo {}: {}", entry.relative_path, e);
        ...
        stats.final_memory_mb = (self.estimate_memory_usage().await as f64 / 1024.0 / 1024.0) as u64;
    ...
    pub async fn clear_all_cache(&self) -> Result<u64> {
        ...
        match self.logo_file_manager.list_files(".").await {
            Ok(files) => {
                ...
                info!("Successfully cleared {} cached logos from filesystem (memory: {})", filesystem_deleted, deleted_count);
            }
            Err(e) => {
