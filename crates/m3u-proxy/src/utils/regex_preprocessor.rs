...
impl RegexPreprocessor {
    ...
    pub fn validate_regex_security(&self, pattern: &str) -> Result<(), String> {
        ...
        if complexity > 50 {
            // Threshold for complex patterns
            return Err(format!(
                ...
                ')' => {
                    if paren_depth > 0 {
                            ...
                            && matches!(next_ch, '*' | '+' | '?' | '{')
                        {
                            ...
                            if paren_depth > 0
                                && quantifier_levels.get(paren_depth - 1) == Some(&true)
                            {
                                return Err(
    ...
    pub fn should_run_regex(
        ...
        let regex_special_chars_present =
            regex_special_chars.iter().any(|&c| field_value.contains(c));
        ...
        let literal_strings_present = if literal_strings
            .iter()
            .any(|s| s.len() >= self.config.minimum_literal_length)
        {
    ...
    fn is_followed_by_optional_quantifier(
        &self,
...
mod tests {
    ...
    fn test_should_run_regex_with_special_chars() {
        ...
        assert!(preprocessor.should_run_regex(
            "This is a sports channel name",
            "channel.*sport.*name",
            "test"
        ));
    }
