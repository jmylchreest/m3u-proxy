...
export function DataMappingExpressionEditor({
  ...
  const [sourceValidations, setSourceValidations] = useState<Map<string, {valid: boolean, preview: any, loading: boolean, error?: string}>>(new Map())
  ...
  Debug.log('DataMapping: Sources debug', {
    ...
    sourceType,
    allSources: allSources.map(s => ({ id: s.id, name: s.name, source_kind: s.source_kind, is_active: s.is_active })),
  ...
  const previewAllSources = useCallback(async () => {
    ...
    await Promise.all(
      sources.map(source => previewSingleSource(source.id))
    )
      ...
      <div className={cn("space-y-4", className)}>
        ...
        <div className="relative">
          <ExpressionEditor
            ...
            placeholder={placeholder || 'Enter data transformation expression (e.g., channel_name = "New " + channel_name)'}
            disabled={disabled}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge 
                className={cn(
                  "gap-1 bg-muted",
                  validationState.isValid === true && "bg-green-500 hover:bg-green-600 text-white border-transparent",
                  validationState.isValid === false && "bg-red-500 hover:bg-red-600 text-white border-transparent"
                )}
              >
            ...
            <TooltipContent>
              <div className="space-y-1">
                ...
                {validationState.isValid === true && (
                  <p>✓ Valid transformation expression</p>
                )}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge 
                className={cn(
                  "gap-1 bg-muted",
                  getCategoryValidationState('syntax').isValid === true && "bg-green-500 hover:bg-green-600 text-white border-transparent",
                  getCategoryValidationState('syntax').isValid === false && "bg-red-500 hover:bg-red-600 text-white border-transparent"
                )}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge 
                className={cn(
                  "gap-1 bg-muted",
                  getCategoryValidationState('field').isValid === true && "bg-green-500 hover:bg-green-600 text-white border-transparent",
                  getCategoryValidationState('field').isValid === false && "bg-red-500 hover:bg-red-600 text-white border-transparent"
                )}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge 
                className={cn(
                  "gap-1 bg-muted",
                  getCategoryValidationState('operator').isValid === true && "bg-green-500 hover:bg-green-600 text-white border-transparent",
                  getCategoryValidationState('operator').isValid === false && "bg-red-500 hover:bg-red-600 text-white border-transparent"
                )}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge 
                className={cn(
                  "gap-1 bg-muted",
                  getCategoryValidationState('value').isValid === true && "bg-green-500 hover:bg-green-600 text-white border-transparent",
                  getCategoryValidationState('value').isValid === false && "bg-red-500 hover:bg-red-600 text-white border-transparent"
                )}
              >
          ...
          {showTestResults && sources.map(source => {
            const previewResult = sourceValidations.get(source.id)
            const fullSourceName = source.name || `Source ${source.id}`
            const sourceName = fullSourceName.length > 8 ? fullSourceName.substring(0, 8) + '...' : fullSourceName
            
            // Determine badge state
            const isLoading = previewResult?.loading
            const hasError = previewResult?.error
            const hasPreview = previewResult && !previewResult.loading && !previewResult.error && previewResult.preview
              ...
              <Tooltip key={source.id}>
                <TooltipTrigger asChild>
                  <Badge 
                    className={cn(
                      "gap-1 bg-muted",
                      isLoading && "bg-yellow-500 hover:bg-yellow-600 text-white border-transparent",
                  ...
                  >
                    ...
                    {sourceName}: {
                      isLoading ? "..." :
                      hasError ? "Error" :
                      hasPreview ? `${previewResult.preview.affected_channels || 0}/${previewResult.preview.total_channels || 0}` :
                      "-/-"
                    }
                  </Badge>
                </TooltipTrigger>
                    ...
                    {hasError ? (
                      ...
                      <div className="space-y-1">
                        {previewResult.preview?.success ? (
                          <p>
                            Success: {previewResult.preview?.message || `Applied to ${previewResult.preview.total_channels || 0} channels`}
                          </p>
                        ) : (
                          <p>
                            Error: {previewResult.preview?.message || 'Unknown error'}
                          </p>
                        )}
                      </div>
                    ) : (
