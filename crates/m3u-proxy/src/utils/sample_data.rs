...
impl SampleDataGenerator {
    ...
    pub fn generate_channel_name(&mut self, category: Option<&str>) -> String {
        ...
        let channel = if let Some(cat) = category {
            ...
            let category_keys: Vec<String> = self
                .data
                .channel_variants
                .categories
                .keys()
    ...
    pub fn generate_sample_channels(
        &mut self,
        count: usize,
        category: Option<&str>,
    ) -> Vec<SampleChannel> {
    ...
    pub fn generate_timeshift_channels(
        &mut self,
        count: usize,
        category: Option<&str>,
    ) -> Vec<SampleChannel> {
    ...
    pub fn generate_standard_channels(
        &mut self,
        count: usize,
        category: Option<&str>,
    ) -> Vec<SampleChannel> {
...
mod tests {
    ...
    fn test_generate_channel_name() {
        ...
        assert!(
            channel.contains("Sports")
                || channel.contains("Racing")
                || channel.contains("Football")
        );
    ...
    fn test_generate_adult_channels() {
        ...
        for channel in &channels {
            assert!(
                channel.channel_name.contains("Adult")
    ...
    fn test_generate_sample_channels() {
        ...
        for (i, channel) in channels.iter().enumerate() {
            ...
            assert!(
                channel.channel_name.contains("Movies") || channel.channel_name.contains("Cinema")
            );
        ...
        for channel in &timeshift_channels {
            assert!(
                channel.channel_name.contains("+1")
                    ...
                    || channel.channel_name.contains("-1h"),
                "Channel '{}' should have timeshift indicator",
                channel.channel_name
            );
        }
        println!(
            "Generated timeshift channels: {:?}",
            timeshift_channels
                .iter()
        ...
        for channel in &standard_channels {
            assert!(
                !channel.channel_name.contains("+1")
                    ...
                    && !channel.channel_name.contains("+24h")
                    && !channel.channel_name.contains("-1h"),
                "Channel '{}' should not have timeshift indicator",
                channel.channel_name
            );
        }
        println!(
            "Generated standard channels: {:?}",
            standard_channels
                .iter()
                .map(|c| &c.channel_name)
                .collect::<Vec<_>>()
    ...
    fn test_generate_with_custom_timeshift_ratio() {
        ...
        let all_timeshift =
            generator.generate_sample_channels_with_options(5, Some("sports"), Some(1.0));
        assert_eq!(all_timeshift.len(), 5);

        let timeshift_count = all_timeshift
            .iter()
        ...
        assert_eq!(
            timeshift_count, 5,
            "All channels should be timeshift with 1.0 ratio"
        );

        // Test 0% timeshift ratio (guaranteed none)
        let no_timeshift =
            generator.generate_sample_channels_with_options(5, Some("news"), Some(0.0));
        let no_timeshift_count = no_timeshift
            .iter()
            .filter(|ch| ch.channel_name.contains("+") || ch.channel_name.contains("-"))
            .count();

        assert_eq!(
            no_timeshift_count, 0,
            "No channels should be timeshift with 0.0 ratio"
        );

        println!(
            "Generated {} timeshift channels out of 5 with 100% ratio",
            timeshift_count
        );
        println!(
            "Generated {} timeshift channels out of 5 with 0% ratio",
            no_timeshift_count
        );
    }
}
