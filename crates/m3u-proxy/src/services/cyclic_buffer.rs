...
impl CyclicBuffer {
    ...
    pub async fn add_client(
        ...
        info!(
            "Added client {} to cyclic buffer. Total clients: {}",
            client.id,
            self.get_client_count().await
        );
    ...
    pub async fn remove_client(&self, client_id: Uuid) -> bool {
        ...
        if removed {
            info!(
                "Removed client {} from cyclic buffer. Total clients: {}",
                client_id,
                clients.len()
            );
        }
    ...
    pub async fn write_chunk(
        ...
        self.bytes_received_from_upstream
        ...
        tracing::trace!(
            "Wrote chunk {} with {} bytes to cyclic buffer",
            sequence,
            data.len()
        );
    ...
    async fn spill_chunk_to_disk(
        &self,
        chunk: &BufferChunk,
    ) -> Result<BufferChunk, Box<dyn std::error::Error + Send + Sync>> {
        let spill_dir = self.spill_dir.as_ref().ok_or("File spill not enabled")?;
        ...
        let spill_file = format!(
            "{}/chunk_{}_{}.dat",
            spill_dir, chunk.sequence, spill_counter
        );
    ...
    async fn read_spilled_chunk(
        &self,
        chunk: &BufferChunk,
    ) -> Result<bytes::Bytes, Box<dyn std::error::Error + Send + Sync>> {
        let spill_path = chunk.spill_path.as_ref().ok_or("Chunk has no spill path")?;
        let temp_manager = self
            .temp_manager
            .as_ref()
            .ok_or("Temp manager not available")?;

        let spill_path_str = spill_path.to_str().ok_or("Invalid spill path")?;
    ...
    fn start_cleanup_task(&self) {
            ...
            .is_ok()
        {
            ...
            let buffer_size_ref =
                Arc::new(AtomicUsize::new(self.buffer_size.load(Ordering::Relaxed)));
    ...
    async fn cleanup_stale_clients(
        ...
        while i < clients_guard.len() {
            if clients_guard[i].is_stale(config.client_timeout).await {
                ...
                warn!(
                    "Removed stale client {} (inactive for {:?})",
    ...
    async fn enforce_buffer_limits(&self) {
        ...
        if current_size > self.config.max_buffer_size {
            ...
            if removed_count > 0 {
                ...
                trace!(
                    "Enforced buffer limits: removed {} chunks ({} bytes)",
                    removed_count, removed_size
                );
            }
        }
    ...
    pub async fn get_connected_clients(&self) -> Vec<ConnectedClient> {
        ...
        for client in clients.iter() {
            ...
            let last_activity =
                now - chrono::Duration::from_std(last_read.elapsed()).unwrap_or_default();

            // Calculate the connected time by subtracting elapsed time from now
            let connected_at =
                now - chrono::Duration::from_std(client.connected_at.elapsed()).unwrap_or_default();
...
mod tests {
    ...
    async fn test_multi_client_buffer() {
        ...
        let client1 = buffer
            .add_client(Some("client1".to_string()), Some("127.0.0.1".to_string()))
            .await;
        let client2 = buffer
            .add_client(Some("client2".to_string()), Some("127.0.0.2".to_string()))
            .await;
