...
impl ChunkSizeManager {
    ...
    pub fn new(default_chunk_size: usize, max_chunk_size: usize) -> Self {
        ...
        stage_dependencies.insert("data_mapping".to_string(), vec!["source_loading".to_string()]);
        stage_dependencies.insert("filtering".to_string(), vec!["source_loading".to_string(), "data_mapping".to_string()]);
        stage_dependencies.insert("logo_prefetch".to_string(), vec!["source_loading".to_string(), "data_mapping".to_string(), "filtering".to_string()]);
        stage_dependencies.insert("channel_numbering".to_string(), vec!["source_loading".to_string(), "data_mapping".to_string(), "filtering".to_string(), "logo_prefetch".to_string()]);
        stage_dependencies.insert("m3u_generation".to_string(), vec!["source_loading".to_string(), "data_mapping".to_string(), "filtering".to_string(), "logo_prefetch".to_string(), "channel_numbering".to_string()]);
        stage_dependencies.insert("epg_processing".to_string(), vec!["source_loading".to_string(), "data_mapping".to_string(), "filtering".to_string()]);
    ...
    pub async fn request_chunk_size(&self, stage: &str, requested_size: usize) -> Result<usize> {
        ...
        if clamped_size != requested_size {
            warn!("Chunk size {} clamped to {} for stage {}", requested_size, clamped_size, stage);
        }
        ...
        let current_max = max_sizes.get(stage).copied().unwrap_or(self.default_chunk_size);
        let new_max = current_max.max(clamped_size);
        
        if new_max > current_max {
            info!("├─ Stage '{}' chunk size increased: {} → {}", stage, current_max, new_max);
            ...
            if let Some(upstream_stages) = self.stage_dependencies.get(stage) {
                for upstream_stage in upstream_stages {
                    let upstream_buffer = buffer_sizes.get(upstream_stage).copied().unwrap_or(self.min_buffer_size);
                    if new_buffer_size > upstream_buffer {
                        info!("└─ Cascading buffer resize to '{}': {} → {}", upstream_stage, upstream_buffer, new_buffer_size);
    ...
    pub async fn get_chunk_size(&self, stage: &str) -> usize {
        ...
        max_sizes.get(stage).copied().unwrap_or(self.default_chunk_size)
    }
    ...
    pub async fn get_buffer_size(&self, stage: &str) -> usize {
        ...
        buffer_sizes.get(stage).copied().unwrap_or(self.min_buffer_size)
    }
    ...
    pub async fn get_stats(&self) -> ChunkSizeStats {
        ...
        ChunkSizeStats {
            ...
            max_chunk_size: max_sizes.values().copied().max().unwrap_or(self.default_chunk_size),
            total_buffer_memory: buffer_sizes.values().sum::<usize>(),
            stage_stats: max_sizes.iter().map(|(stage, &chunk_size)| {
                let buffer_size = buffer_sizes.get(stage).copied().unwrap_or(self.min_buffer_size);
                (stage.clone(), StageChunkStats { chunk_size, buffer_size })
            }).collect(),
        }
    }
