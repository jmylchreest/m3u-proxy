...
impl PublishContentStage {
    ...
    pub async fn process(
        ...
        for (artifact_index, artifact) in input_artifacts.into_iter().enumerate() {
            let progress_percentage =
                50.0 + (artifact_index as f64 / total_artifacts as f64 * 40.0); // 50% to 90%
    ...
    async fn publish_file_artifact(&self, artifact: PipelineArtifact) -> Result<PipelineArtifact> {
        ...
        if self.enable_versioning {
            debug!(
                "Versioning enabled - creating backup for {}",
                target_filename
            );
            self.create_backup(&target_filename).await?;
        } else {
            debug!(
                "Versioning disabled - no backup created for {}",
                target_filename
            );
        }

        // Perform atomic move from temporary to final location
        self.atomic_move(&artifact.file_path, &target_filename)
            .await?;
        ...
        .with_metadata(
            "original_temp_file".to_string(),
            artifact.file_path.clone().into(),
        )
        .with_metadata(
            "publish_duration_ms".to_string(),
            publish_duration.as_millis().to_string().into(),
        );
    ...
    async fn create_backup(&self, target_filename: &str) -> Result<()> {
        ...
        let exists: bool =
            (self.proxy_output_file_manager.exists(target_filename).await).unwrap_or_default();
        ...
        if exists {
            ...
            // Write backup file
            self.proxy_output_file_manager
                .write(&backup_filename, &existing_content)
                .await
    ...
    async fn atomic_move(&self, temp_file_path: &str, target_filename: &str) -> Result<()> {
        ...
        let content = self
            ...
            .await
            .map_err(|e| {
                anyhow::anyhow!("Failed to read temporary file '{}': {}", temp_file_path, e)
            })?;
        ...
        self.proxy_output_file_manager
            .write(target_filename, &content)
            .await
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to write to final location '{}': {}",
                    target_filename,
                    e
                )
            })?;
    ...
    pub async fn cleanup_published_files(
        &self,
        ...
        for artifact in published_artifacts {
            if matches!(artifact.artifact_type.stage, ProcessingStage::Published) {
                    ...
                    .await
                {
                    ...
                    Err(e) => warn!(
                        "Failed to clean up published file '{}': {}",
...
impl PipelineStage for PublishContentStage {
    async fn execute(
        ...
        self.report_progress(25.0, "Initializing file publishing")
            .await;
        let result = self.process(input).await.map_err(|e| {
            PipelineError::stage_error("publish_content", format!("Publishing failed: {e}"))
        })?;
