...
use crate::{
    models::relay::{ResolvedRelayConfig, HwAccelCapabilities, VideoCodec, AudioCodec},
...
impl FFmpegCommandBuilder {
    ...
    pub fn build_args(
        ...
        if !config.effective_args.is_empty() {
            return self.substitute_templates(&config.effective_args, input_url, output_path, config);
        }
    ...
    pub async fn create_mapping_strategy(
        &self,
        input_url: &str,
    ) -> Option<StreamMappingStrategy> {
        if let Some(ref prober) = self.stream_prober {
            match prober.probe_input(input_url).await {
                Ok(probe_result) => {
                    debug!("Stream probe successful: {} streams found", 
    ...
    fn add_stream_mapping(&self, args: &mut Vec<String>, mapping_strategy: Option<&StreamMappingStrategy>) {
    ...
    fn add_video_codec_args(
        ...
        let should_copy = mapping_strategy
            .map(|s| s.video_copy)
            .unwrap_or(false);
    ...
    fn add_explicit_audio_params(&self, args: &mut Vec<String>, profile: &crate::models::relay::RelayProfile) {
    ...
    fn should_apply_hwaccel_filters(
        ...
        if let Some(strategy) = mapping_strategy {
            profile.enable_hardware_acceleration && strategy.video_mapping.is_some() && !strategy.video_copy
        } else {
    ...
    fn substitute_templates(
        ...
        for arg in effective_args {
            let resolved = arg
                ...
                .replace("{segment_duration}", &config.profile.segment_duration.unwrap_or(30).to_string())
                .replace("{max_segments}", &config.profile.max_segments.unwrap_or(4).to_string())
