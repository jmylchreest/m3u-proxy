...
async function setupConsoleCapture(page: Page): Promise<{ logs: string[], errors: string[], networkErrors: string[] }> {
  const logs: string[] = [];
  const errors: string[] = [];
...
test.describe('Data Mapping Rule Validation Tests', () => {
  ...
  test('should test expression validation workflow', async ({ page }) => {
    ...
    const errorIndicators = page.locator('[class*="red"], [class*="error"], [class*="destructive"]');
  ...
  test('should test editing existing rule (if any exist)', async ({ page }) => {
    ...
    const editButtons = page.locator('button:has([class*="edit"], [data-testid="edit"]), button[title*="edit" i]');
