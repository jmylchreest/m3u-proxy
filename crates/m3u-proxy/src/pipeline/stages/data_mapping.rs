...
use crate::pipeline::models::{ArtifactType, PipelineArtifact};
...
use crate::pipeline::engines::{
    ...
    ProgramDataMappingEngine, StreamRuleProcessor,
};
...
impl DataMappingStage {
    ...
    pub async fn process_channels(
        ...
        let mut rule_stats: HashMap<String, (String, usize, usize, std::time::Duration)> =
        ...
        for source in stream_sources {
            ...
            let output_file_path =
                format!("{}_mapping_channels.jsonl", self.pipeline_execution_prefix);
            ...
            for channel_model in channel_models {
                match self.create_channel_from_model(&channel_model) {
                    Ok(channel) => {
                        ...
                        if current_batch.len() >= BATCH_SIZE {
                            if !current_batch.is_empty() {
                                let batch_result =
                                    if let Some((ref mut engine, ref rule_name_map)) = engine {
                                        // Process batch through engine
                                        let result = engine
                                            .process_records(std::mem::take(&mut current_batch))?;
                                        ...
                                        for (rule_id, results) in &result.rule_results {
                                            let applied_count =
                                                results.iter().filter(|r| r.rule_applied).count();
                                            let total_time: std::time::Duration =
                                                results.iter().map(|r| r.execution_time).sum();
                                            let rule_name = rule_name_map
                                                .get(rule_id)
                                                .cloned()
                                                .unwrap_or_else(|| rule_id.clone());

                                            let entry =
                                                rule_stats.entry(rule_id.clone()).or_insert((
                                                    rule_name.clone(),
                                                    0,
                                                    0,
                                ...
                                for processed_channel in batch_result.processed_records {
                                    if let Some(ref helper_processor) = self.helper_processor {
                                        if helper_processor
                                            .record_needs_processing(&processed_channel)
                                        {
                                            let channel_name =
                                                processed_channel.channel_name.clone();
                                            match helper_processor
                                                .process_record(processed_channel)
                                                .await
                                            {
                                                Ok((processed_record, modifications)) => {
                                                    if !modifications.is_empty() {
                                                        ...
                                                        trace!(
                                                            "Applied {} helper modifications to channel {}",
                                                            modifications.len(),
                                                            processed_record.channel_name
                                                        );
                                                    }
                                                    final_channels.push(processed_record);
                                                }
                                                Err(
                                                    HelperProcessorError::CriticalDatabaseError(
                                                        ref db_error,
                                                    ),
                                                ) => {
                                                    // Critical database error - halt the entire pipeline
                                                    error!(
                                                        "Critical database error during helper processing for channel {}: {}",
                                                ...
                                                Err(e) => {
                                                    ...
                                                    warn!(
                                                        "Helper processing failed for channel {}: {}",
                                                        channel_name, e
                                                    );
                                                    warn!(
                                                        "Skipping channel {} due to helper processing error",
                                                        channel_name
                                                    );
                                                }
                                            }
                                ...
                                if helper_modifications > 0 {
                                    ...
                                    trace!(
                                        "Helper processor applied {} modifications to batch",
                                        helper_modifications
                                    );
                                }
                                ...
                                if !batch_content.is_empty() {
                                    if !output_file_created {
                                        ...
                                        self.file_manager
                                            .write(&output_file_path, batch_content.as_bytes())
                                            .await?;
                                        output_file_created = true;
                                    } else {
                                        // Subsequent writes - append
                                        let existing_content =
                                            self.file_manager.read(&output_file_path).await?;
                                        let mut combined_content =
                                            String::from_utf8(existing_content)?;
                                        combined_content.push_str(&batch_content);
                                        self.file_manager
                                            .write(&output_file_path, combined_content.as_bytes())
                                            .await?;
                                    }
                                    batch_content.clear();
                                }
                    ...
                    Err(e) => {
                        error!(
                            "Failed to create channel from model {}: {}",
                            channel_count + 1,
                            e
                        );
                        return Err(e.into());
            ...
            if !current_batch.is_empty() {
                let batch_result = if let Some((ref mut engine, ref rule_name_map)) = engine {
                    ...
                    for (rule_id, results) in &result.rule_results {
                        ...
                        let entry = rule_stats.entry(rule_id.clone()).or_insert((
                            rule_name.clone(),
                            0,
                            0,
                            std::time::Duration::ZERO,
                ...
                if helper_modifications > 0 {
                    source_modified_count += helper_modifications;
                    trace!(
                        "Helper processor applied {} modifications to final batch",
                        helper_modifications
                    );
                }
            ...
            info!(
                ...
                format_duration_precise(source_duration)
            );
        ...
        if !rule_stats.is_empty() {
            ...
            for (rule_id, (rule_name, applied_count, processed_count, total_time)) in rule_stats {
                ...
                info!(
                    ...
                    processed_count,
                    affected_percent,
                    format_duration_precise(avg_time)
                );
            }
        }
        ...
        .with_metadata(
            "total_modified".to_string(),
            serde_json::Value::Number(serde_json::Number::from(total_channels_modified)),
        )
        .with_metadata(
            "stage_duration_ms".to_string(),
            serde_json::Value::Number(serde_json::Number::from(stage_duration.as_millis() as u64)),
        );
    ...
    pub async fn process_programs(
        ...
        let program_count = if epg_rules.is_empty() {
            ...
            let processed_programs = if let Some(ref helper_processor) = self.helper_processor {
                        ...
                        match helper_processor.process_record(program.clone()).await {
                            Ok((processed_program, modifications)) => {
                                if !modifications.is_empty() {
                                    helper_modifications += 1;
                                    trace!(
                                        "Applied {} helper modifications to EPG program {}",
                                        modifications.len(),
                                        processed_program.title
                                    );
                                }
                ...
                if helper_modifications > 0 {
                    info!(
                        "Applied helper processing to {} EPG programs",
                        helper_modifications
                    );
                }
            ...
            self.write_programs_to_file(&processed_programs, &output_file_path)
                .await?;

            count
        } else {
            info!(
                "Found {} EPG data mapping rules, processing EPG programs with rules",
                epg_rules_count
            );
            ...
            for (rule_id, rule_results) in &engine_result.rule_results {
                if let Some((rule_name, applied_count, processed_count, total_time)) =
                    rule_stats.get_mut(rule_id)
                {
                    ...
                    *total_time = rule_results.iter().map(|r| r.execution_time).sum();
                    ...
                    info!(
                        "Rule '{}' applied to {}/{} EPG programs in {}",
                        rule_name,
                        applied_count,
                        processed_count,
            ...
            info!(
                "EPG data mapping engine processed {} programs, {} total modified",
        ...
        .with_metadata(
            "epg_rules_found".to_string(),
            serde_json::Value::Number(serde_json::Number::from(epg_rules_count)),
        );
    ...
    async fn write_programs_to_file(
        &self,
        programs: &[EpgProgram],
        file_path: &str,
    ...
    fn create_channel_from_model(
        &self,
        model: &crate::entities::channels::Model,
    ...
    fn create_data_mapping_rule_from_model(
        &self,
        model: &crate::entities::data_mapping_rules::Model,
    ) -> Result<DataMappingRule, anyhow::Error> {
    ...
    fn create_epg_program_from_model(
        &self,
        model: &crate::entities::epg_programs::Model,
    ) -> Result<EpgProgram, anyhow::Error> {
    ...
    fn log_rule_results<T>(&self, result: &EngineResult<T>, processed_count: usize) {
        if !result.rule_results.is_empty() {
            trace!(
                "Engine batch result: batch_end_channel={} batch_processed={} batch_modified={} batch_duration={}",
                processed_count,
                result.total_processed,
                result.total_modified,
                format_duration_precise(result.execution_time)
            );
            ...
            for (rule_id, rule_results) in &result.rule_results {
                ...
                if applied_count > 0 {
                    trace!(
                        "Rule batch result: rule_id={} applied_count={} batch_records={}",
                        rule_id,
                        applied_count,
                        rule_results.len()
                    );
                    ...
                    for (i, rule_result) in rule_results.iter().enumerate() {
                        if rule_result.rule_applied && !rule_result.field_modifications.is_empty() {
                            trace!(
                                "  Record modification: record_index={} field_count={} duration={}",
                                i + 1,
                                rule_result.field_modifications.len(),
                                format_duration_precise(rule_result.execution_time)
                            );
                            for modification in &rule_result.field_modifications {
                                trace!(
                                    "    Field change: field={} action={} old_value={:?} new_value={:?}",
...
impl PipelineStage for DataMappingStage {
    async fn execute(
        ...
        let channel_artifact = self.process_channels().await.map_err(|e| {
            PipelineError::stage_error("data_mapping", format!("Channel processing failed: {e}"))
        })?;
        artifacts.push(channel_artifact);

        // Process EPG programs
        self.report_progress(50.0, "Processing EPG programs").await;
        let program_artifact = self.process_programs().await.map_err(|e| {
            PipelineError::stage_error("data_mapping", format!("EPG processing failed: {e}"))
        })?;
