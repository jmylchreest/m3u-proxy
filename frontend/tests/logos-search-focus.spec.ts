...
test.describe('Logos Search Focus Investigation', () => {
  ...
  test('should maintain focus during debounced search', async ({ page }) => {
    ...
    for (let i = 0; i < searchTerm.length; i++) {
      ...
      console.log(`[TEST] After typing "${searchTerm.slice(0, i + 1)}", focus maintained: ${isFocused}`);
    ...
    const resultsCount = await page.locator('.space-y-4 > .transition-all, .grid > .transition-all, .space-y-2 > .transition-all').count();
  ...
  test('should handle rapid typing without losing focus', async ({ page }) => {
    ...
    console.log(`[TEST] Rapid typing completed. Focus lost during typing: ${focusLostDuringTyping} at position ${focusLostAt}`);
    console.log(`[TEST] Final focus state: ${finalFocusState}`);
  ...
  test('should verify search input ref and focus restoration logic', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      document.addEventListener('focusin', (e) => {
        focusEvents.push({
          event: 'focusin',
          timestamp: Date.now(),
          element: (e.target as HTMLElement)?.tagName + (e.target as HTMLElement)?.className ? '.' + (e.target as HTMLElement).className : ''
        });
      });
      ...
      (window as any).clearFocusEvents = () => { focusEvents = []; };
    });
    ...
    console.log(`[TEST] Focus out events: ${focusOutEvents.length}, Focus in events: ${focusInEvents.length}`);
  ...
  test('should verify component re-render behavior', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      (window as any).incrementRenderCount = () => { renderCount++; };
    });
  ...
  test('should test search input with various view modes', async ({ page }) => {
    const searchInput = page.locator('[placeholder*="Search logos"]');
    const gridViewButton = page.locator('[data-testid="grid-view"], button:has-text("Grid"), button:has([class*="grid"])', { hasText: '' }).first();
    const listViewButton = page.locator('[data-testid="list-view"], button:has-text("List"), button:has([class*="list"])', { hasText: '' }).first();
    const tableViewButton = page.locator('[data-testid="table-view"], button:has-text("Table"), button:has([class*="table"])', { hasText: '' }).first();
