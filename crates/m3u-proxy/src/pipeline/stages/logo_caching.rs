...
impl LogoCachingStage {
    ...
    async fn report_progress(&self, percentage: f64, message: &str) {
        if let Some(pm) = &self.progress_manager
            && let Some(updater) = pm.get_stage_updater("logo_caching").await
        {
    ...
    pub async fn process_channels(
        ...
        if !self.file_manager.exists(&input_file_path).await? {
            warn!(
                "No filtered channels file found at {}, skipping logo caching",
                input_file_path
            );
            let output_path = format!(
                "{}_logo_cached_channels.jsonl",
        ...
        if !self.config.cache_channel_logos {
            ...
            self.write_channels_to_file(&channels, &output_file_path)
                .await?;
        ...
        self.write_channels_to_file(&result.processed_records, &output_file_path)
            .await?;
    ...
    async fn process_channel_logos(
        &self,
        channels: Vec<Channel>,
    ) -> Result<LogoCachingResult, Box<dyn std::error::Error>> {
                ...
                if let Some(ref logo_url) = original_logo {
                    match self.classify_logo_url(logo_url) {
                        LogoUrlType::RemoteUrl => {
                            remote_urls += 1;
                            trace!(
                                "Processing remote logo URL for channel {}: {}",
                            ...
                            let logo_exists = self
                                .file_manager
                                .exists(&logo_file_path)
                                .await
                                .unwrap_or(false);
                            ...
                            if logo_exists {
                                cache_hits += 1;
                                trace!(
                                    "Cache hit for channel {}: {} (existing cache_id={})",
                                ...
                                .await
                            {
                                Ok((returned_cache_id, bytes_transferred)) => {
                                    let new_url = self.logo_service.get_cached_logo_url(
                                        &returned_cache_id,
                                    ...
                                    trace!(
                                        "Processed logo for channel {}: {} -> cache_id={} (hit={} bytes={})",
                                ...
                                Err(e) => {
                                    cache_failures += 1;
                                    debug!(
                                        "Failed to cache logo for channel {}: {} - Error: {}",
                        ...
                        LogoUrlType::LocalProxy => {
                            local_proxy_urls += 1;
                            trace!(
                                "Skipping local proxy URL for channel {}: {}",
                                channel.channel_name, logo_url
                            );
                            // Keep as-is
                        }
                        LogoUrlType::Unknown => {
                            unknown_urls += 1;
                            trace!(
                                "Skipping unknown URL format for channel {}: {}",
                                channel.channel_name, logo_url
                            );
                            // Keep as-is
                        }
            ...
            if should_log_progress || is_final_batch {
                ...
                let estimated_remaining = if processed_count > 0 && processed_count < total_channels
                {
                    ...
                    std::time::Duration::from_secs_f32(
                        avg_time_per_channel * (total_channels - processed_count) as f32,
                    )
                } else {
                ...
                let progress_message = format!(
                    "Caching logos: {processed_count}/{total_channels} channels ({progress_pct:.1}%)"
                );
                self.report_progress(adjusted_progress, &progress_message)
                    .await;
            } else {
                // Just log batch completion without full progress details
                debug!(
                    "Completed logo caching batch {}/{}: processed={} duration={}",
                    batch_index + 1,
                    total_batches,
                    batch.len(),
    ...
    async fn read_channels_from_file(
        &self,
        file_path: &str,
    ...
    async fn write_channels_to_file(
        &self,
        channels: &[Channel],
        file_path: &str,
    ...
    pub async fn process(
        ...
        info!(
            "Starting logo caching stage with {} input artifacts",
            input_artifacts.len()
        );
        ...
        for (artifact_index, artifact) in input_artifacts.into_iter().enumerate() {
            ...
            let processed_artifact = match artifact.artifact_type.content {
                ...
                _ => {
                    ...
                    .await;
                    debug!(
                        "Passing through artifact of type {:?} unchanged",
    ...
    async fn process_channel_artifact(
        ...
        self.write_channels_to_file(&logo_result.processed_records, &output_file_path)
            .await?;
        ...
        .with_metadata(
            ...
            serde_json::Value::Number(serde_json::Number::from(logo_result.total_cached)),
        )
        .with_metadata(
            "cache_failures".to_string(),
            serde_json::Value::Number(serde_json::Number::from(logo_result.cache_failures)),
        )
        .with_metadata(
            "cache_hits".to_string(),
            serde_json::Value::Number(serde_json::Number::from(logo_result.cache_hits)),
        )
        .with_metadata(
            "total_downloaded_bytes".to_string(),
            serde_json::Value::Number(serde_json::Number::from(logo_result.total_downloaded_bytes)),
        );
    ...
    async fn process_epg_artifact(
        ...
        if !self.config.cache_program_logos {
            ...
            self.write_programs_to_file(&programs, &output_file_path)
                .await?;
        ...
        self.write_programs_to_file(&logo_result.processed_records, &output_file_path)
            .await?;
    ...
    async fn read_programs_from_file(
        &self,
        file_path: &str,
    ...
    async fn process_program_logos(
        &self,
        programs: Vec<EpgProgram>,
    ) -> Result<EpgLogoCachingResult, Box<dyn std::error::Error>> {
                    ...
                    LogoUrlType::RemoteUrl => {
                            ...
                            .await
                        {
                            Ok((cache_id, bytes_transferred)) => {
                                let cached_url = self
                                    .logo_service
                                    .get_cached_logo_url(&cache_id, &self.config.base_url);
                                program.program_icon = Some(cached_url);
                                total_cached += 1;
                                total_downloaded_bytes += bytes_transferred;

                                trace!(
                                    "Program icon downloaded and cached: {} -> {}",
                                    program_icon_url,
                                    program.program_icon.as_ref().unwrap()
                                );
                            }
                            Err(e) => {
...
impl PipelineStage for LogoCachingStage {
    async fn execute(
        ...
        let result = self.process(input).await.map_err(|e| {
            PipelineError::stage_error("logo_caching", format!("Logo caching failed: {e}"))
        })?;
...
mod tests {
    ...
    fn test_logo_url_classification() {
        ...
        assert_eq!(
            classify_test_url(
                "https://proxy.example.com/api/v1/logos/cached/cache123",
                base
            ),
            LogoUrlType::LocalProxy
        );
    ...
    fn test_channel_logo_processing_logic() {
        let _channels = vec![
            ...
            create_test_channel(
                "ITV",
                Some("https://proxy.example.com/api/v1/logos/uuid123".to_string()),
            ),
            create_test_channel(
                "Channel 4",
