...
use crate::utils::{
    CompressionFormat, DecompressionService, HttpClientFactory, StandardHttpClient,
};
    ...
    async fn fetch_xmltv_content(&self, url: &str) -> AppResult<String> {
        ...
        let decompressed_bytes = match compression_format {
            ...
            _ => {
                ...
                DecompressionService::decompress(bytes.into()).map_err(|e| {
                    AppError::source_error(format!("Failed to decompress XMLTV content: {e}"))
                })?
            }
        };
        ...
        let content = String::from_utf8(decompressed_bytes).map_err(|e| {
            AppError::source_error(format!("Failed to decode XMLTV content as UTF-8: {e}"))
        })?;

        debug!(
            "Successfully processed {} bytes of XMLTV content (compression: {:?})",
            content.len(),
            compression_format
        );
        Ok(content)
    ...
    async fn validate_epg_source(&self, source: &EpgSource) -> AppResult<SourceValidationResult> {
        ...
        if source.source_type != EpgSourceType::Xmltv {
            return Ok(SourceValidationResult::failure(vec![format!(
                "Expected XMLTV source type, got {:?}",
                source.source_type
            )]));
        }
        ...
        if source.url.is_empty() {
            validation
                .errors
                .push("URL is required for XMLTV sources".to_string());
        ...
        if source.username.is_some() || source.password.is_some() {
            validation =
                validation.with_warning("XMLTV sources typically don't require authentication");
        }
...
impl EpgProgramIngestor for XmltvEpgHandler {
    ...
    async fn estimate_program_count(&self, source: &EpgSource) -> AppResult<Option<u32>> {
        ...
        debug!(
            "Program count estimation not available for XMLTV source: {}",
            source.name
        );
        Ok(None)
    }
    ...
    async fn ingest_epg_programs_with_progress_updater(
        ...
        info!(
            "Starting XMLTV EPG ingestion with ProgressStageUpdater for source: {}",
            source.name
        );
        ...
        if let Some(updater) = progress_updater {
            updater
                .update_progress(
                    0.0,
                    &format!("Starting XMLTV ingestion for {}", source.name),
                )
                .await;
        }
        ...
        if let Some(updater) = progress_updater {
            updater
                .update_progress(
                    100.0,
                    &format!("Completed: {} programs ingested", programs.len()),
                )
                .await;
        ...
        info!(
            "XMLTV EPG ingestion completed for source: {} ({} programs)",
            source.name,
            programs.len()
        );
        Ok(programs)
