...
impl NumberingStage {
    ...
    async fn report_progress(&self, percentage: f64, message: &str) {
        if let Some(pm) = &self.progress_manager
            && let Some(updater) = pm.get_stage_updater("numbering").await
        {
    ...
    pub async fn process(
        ...
        info!(
            "Starting numbering stage with {} input artifacts",
            input_artifacts.len()
        );
        ...
        for (artifact_index, artifact) in input_artifacts.into_iter().enumerate() {
            let progress_percentage =
                40.0 + (artifact_index as f64 / total_artifacts as f64 * 50.0); // 40% to 90%
            ...
            match artifact.artifact_type.content {
                ContentType::Channels => {
                    ...
                    .await;
                    let (numbered_artifact, processed, assigned, conflicts) =
        ...
        self.report_progress(
            95.0,
            &format!("Finalizing: {total_numbers_assigned} channels numbered"),
        )
        .await;
        info!(
            ...
            total_channo_conflicts_resolved
        );
    ...
    async fn process_channel_artifact(
    ...
    ) -> Result<(PipelineArtifact, usize, usize, usize), Box<dyn std::error::Error>> {
        ...
        let (assigned_count, channo_conflicts_resolved) =
            self.apply_numbering(&mut channels).await?;
        ...
        self.file_manager
            .write(&output_filename, output_content.as_bytes())
            .await?;
        ...
        .with_metadata(
            "channels_assigned_numbers".to_string(),
            assigned_count.into(),
        )
        .with_metadata(
            "channo_conflicts_resolved".to_string(),
            channo_conflicts_resolved.into(),
        )
        .with_metadata(
            "starting_channel_number".to_string(),
            self.starting_channel_number.into(),
        );
    ...
    async fn copy_epg_artifact(
        ...
        debug!(
            "Copied EPG programs artifact: {} -> {}",
            artifact.file_path, output_filename
        );
    ...
    async fn apply_numbering(
        ...
        if total_channels > 1000 {
            debug!(
                "Numbering progress: starting analysis of {} channels (0.0%), elapsed: {}",
                total_channels,
                format_duration_precise(algorithm_start.elapsed())
            );
            ...
            progress_task = Some(tokio::spawn(async move {
                ...
                loop {
                    ...
                    debug!(
                        "Numbering progress: processing {} channels, elapsed: {}",
                        total_for_task,
                ...
                if let Ok(mut desired_channo) = channo_str.trim_start_matches('0').parse::<u32>() {
                    if desired_channo > 0 {
                        ...
                        if desired_channo != original_channo {
                            warn!(
                                "Channel number conflict {} found for channel '{}', reassigned to {}",
                    ...
                    // removed redundant continue (clippy::needless_continue)
                        ...
                        debug!(
                            "Invalid channel number 0 for channel '{}', will assign new number",
                    ...
                    debug!(
                        "Invalid channel number format '{}' for channel '{}', will assign new number",
        ...
        if total_channels > 1000 {
            debug!(
                "Numbering progress: first pass complete, {} channels analyzed (33.3%), elapsed: {}",
                total_channels,
                format_duration_precise(algorithm_start.elapsed())
            );
            ...
            let progress_message =
                format!("First pass complete: {total_channels} channels analyzed");
        ...
        let sequential_assignment_needed = channels_needing_numbers
            .iter()
        ...
        if total_channels > 1000 {
            debug!(
                "Numbering progress: pool building complete, {} available numbers (66.7%), elapsed: {}",
                available_numbers.len(),
                format_duration_precise(algorithm_start.elapsed())
            );
            ...
            let progress_message = format!(
                "Pool building complete: {} available numbers",
                available_numbers.len()
            );
            ...
            match assigned_num {
                ...
                None => {
                    ...
                    if let Some(next_number) = available_numbers.pop_first() {
                        ...
                        return Err(format!(
                            "Ran out of available channel numbers starting from {}",
                            self.starting_channel_number
                        )
                        .into());
                    }
                }
        ...
        if total_channels > 1000 {
            debug!(
                "Numbering progress: assignment complete, {} channels processed (100.0%), elapsed: {}",
                total_channels,
            ...
            // Broadcast progress update via SSE
            let progress_message =
                format!("Assignment complete: {total_channels} channels processed");
        ...
        let (min_num, max_num) = if !used_numbers.is_empty() {
            (
                *used_numbers.iter().min().unwrap(),
                *used_numbers.iter().max().unwrap(),
            )
        } else {
        ...
        if channels_with_existing_channo > 0 {
            let existing_channo_percentage =
                (channels_with_existing_channo as f64 / total_channels as f64) * 100.0;
            info!(
...
impl PipelineStage for NumberingStage {
    async fn execute(
        ...
        self.report_progress(20.0, "Initializing channel numbering")
            .await;
        let result = self.process(input).await.map_err(|e| {
            PipelineError::stage_error("numbering", format!("Numbering failed: {e}"))
        })?;
        self.report_progress(100.0, "Channel numbering completed")
            .await;
        Ok(result)
    }
