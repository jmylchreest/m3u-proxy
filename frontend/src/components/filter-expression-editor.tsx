...
export function FilterExpressionEditor({
  ...
  const [sourceValidations, setSourceValidations] = useState<Map<string, {valid: boolean, matchCount: number, totalCount: number, loading: boolean, error?: string}>>(new Map())
  ...
  const validateAllSources = useCallback(async () => {
    ...
    await Promise.all(
      sources.map(source => validateSingleSource(source.id))
    )
            ...
            <TooltipContent>
              <div className="space-y-1">
                ...
                {validationState.isValid === null && (
                  <p>Enter expression to validate</p>
                )}
              </div>
            </TooltipContent>
          </Tooltip>
              ...
              <Tooltip key={source.id}>
                <TooltipTrigger asChild>
                  <Badge 
                    className={cn(
                      "gap-1 bg-muted",
                      isTestedSuccessfully && "bg-green-500 hover:bg-green-600 text-white border-transparent",
                  ...
                  >
                    ...
                    {sourceName}: {
                      isLoading ? "Testing..." :
                      hasError ? "Error" :
                      isTestedSuccessfully ? `${testResult.matchCount}/${testResult.totalCount}` :
                      "-/-"
                    }
                  </Badge>
                </TooltipTrigger>
                <TooltipContent>
                  <div className="space-y-1">
                    ...
                    {isLoading && (
                      <p>Testing filter against source...</p>
                    )}
                    {hasError ? (
                      ...
                      <div>
                        ...
                        <p>Percentage: {testResult.totalCount > 0 ? Math.round((testResult.matchCount / testResult.totalCount) * 100) : 0}%</p>
                      </div>
                    ) : (
