...
export function Proxies() {
  ...
  useEffect(() => {
    ...
    const handleGlobalProxyEvent = (event: any) => {
      ...
      if ((event.state === 'idle' || event.state === 'processing') && event.id && event.operation_type === 'proxy_regeneration') {
      ...
      if ((event.state === 'completed' || event.state === 'error') && event.id && event.operation_type === 'proxy_regeneration') {
  ...
  const handleRegenerateProxy = async (proxyId: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        console.log(`[Proxies] Fallback timeout - clearing regenerating state for proxy: ${proxyId}`)
        setRegeneratingProxies(prev => {
          const newSet = new Set(prev)
              ...
              {(filteredProxies?.length === 0) && !loading.proxies && (
                <div className="text-center py-8">
                  ...
                  <h3 className="mt-4 text-lg font-semibold">
                    {searchTerm || filterStatus !== "all" ? "No matching proxies" : "No proxies found"}
                  </h3>
