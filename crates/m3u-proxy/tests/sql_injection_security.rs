...
use anyhow::Result;
use sea_orm::{
    ActiveModelTrait, ColumnTrait, DatabaseConnection, DbErr, EntityTrait, MockDatabase,
    MockExecResult, Order, PaginatorTrait, QueryFilter, QueryOrder, Set, TransactionTrait,
};
use uuid::Uuid;
...
impl TestDataFactory {
    ...
    fn create_stream_source_request(
        name: &str,
        malicious_input: Option<&str>,
    ...
    fn create_bulk_stream_source_requests(
        count: usize,
        with_malicious: bool,
    ) -> Vec<StreamSourceCreateRequest> {
        (0..count)
            .map(|i| {
...
impl SecurityTestHelper {
    ...
    fn assert_no_sql_syntax_error(error: &anyhow::Error) {
        ...
        assert!(
            ...
            "SQL syntax error indicates possible injection: {}",
            error_msg
        );
        assert!(
            !error_msg.contains("sql syntax"),
            "SQL syntax error indicates possible injection: {}",
            error_msg
        );
        assert!(
            !error_msg.contains("near"),
            "SQL 'near' error may indicate injection attempt: {}",
            error_msg
        );
    }
    ...
    fn assert_no_sql_injection_in_db_error(error: &DbErr) {
        ...
        assert!(
            !error_msg.contains("syntax error"),
            "SQL syntax error in DbErr indicates possible injection: {}",
            error_msg
        );
        assert!(
            !error_msg.contains("sql syntax"),
    ...
    fn assert_malicious_input_stored_safely(stored_value: &str, original_malicious_input: &str) {
        assert_eq!(
            stored_value, original_malicious_input,
    ...
    fn assert_no_sensitive_info_leaked(error_msg: &str) {
        ...
        assert!(
            !lowercase_msg.contains("password"),
            "Error message leaks password information"
        );
        assert!(
            !lowercase_msg.contains("secret"),
            "Error message leaks secret information"
        );
        assert!(
            !lowercase_msg.contains("private_key"),
            "Error message leaks private key information"
        );
        assert!(
            !lowercase_msg.contains("connection_string"),
            "Error message leaks connection string"
        );
        assert!(
            !lowercase_msg.contains("database schema"),
            "Error message leaks schema information"
        );
        assert!(
            !lowercase_msg.contains("table structure"),
            "Error message leaks table structure"
        );
        assert!(
            !lowercase_msg.contains("sql query"),
            "Error message leaks SQL query details"
        );
        assert!(
            !lowercase_msg.contains("bind parameter"),
            "Error message leaks parameter details"
        );
    }
}
    ...
    for &payload in SQL_INJECTION_PAYLOADS {
        ...
        match count_result {
            Ok(count) => {
                ...
                assert_eq!(
                    count, 0,
                    "Count query should safely return 0 for malicious input: {}",
                    payload
                );
            }
            Err(e) => {
        ...
        let test_request =
            TestDataFactory::create_stream_source_request("Test Source", Some(payload));
        ...
        match insert_result {
            Ok(inserted_model) => {
                ...
                SecurityTestHelper::assert_malicious_input_stored_safely(
                    &inserted_model.name,
                    payload,
                );
...
async fn test_seaorm_order_by_injection_prevention() -> Result<()> {
    ...
    for &pattern in &malicious_order_patterns {
        ...
        assert!(
            result_asc.is_ok(),
            "Type-safe ascending order should always work"
        );
        ...
        assert!(
            result_desc.is_ok(),
            "Type-safe descending order should always work"
        );
        ...
        let result_created = StreamSources::find()
            ...
            .await;
        assert!(
            result_created.is_ok(),
            "Type-safe timestamp ordering should always work"
        );
        ...
        println!(
            "[OK] Malicious ORDER BY pattern '{}' cannot be injected due to SeaORM type safety",
            pattern
        );
    }
    ...
    for column_name in valid_sort_columns {
        ...
        assert!(
            sort_result.is_ok(),
            "Valid column sorting should work: {}",
            column_name
        );
    }
    ...
    assert!(
        safe_sort_result.is_none(),
        "Malicious sort column should be safely ignored"
    );
        ...
        match result {
            Ok(source) => {
                ...
                match updated {
                    Ok(updated_source) => {
                        assert!(
                            updated_source.name.contains(payload),
                ...
                let url_search_result = repo
                    .find_by_url_and_type(&malicious_url, StreamSourceType::M3u)
                    .await;
...
async fn test_seaorm_transaction_safety_with_malicious_input() -> Result<()> {
                    ...
                    Ok(inserted_model) => {
                        ...
                        if malicious_payload.contains("DROP") {
                            ...
                            return Err(DbErr::Custom(
                                "Business logic rollback for malicious content".to_string(),
                            ));
                        }
    ...
    match tx_result {
        Ok(_) => {
            ...
            let count = StreamSources::find()
                .paginate(connection.as_ref(), 1)
                .num_items()
                .await?;
            assert_eq!(
                count, 2,
                "Both records should exist if transaction committed"
            );
            ...
            assert!(
                malicious_record.is_some(),
                "Malicious data should be stored safely"
            );
                ...
                for (i, request) in malicious_requests.into_iter().enumerate() {
                    ...
                    match insert_result {
                        Ok(inserted) => {
                            ...
                            if i % 2 == 0 {
                                // Even indices have malicious input
                                let payload =
                                    SQL_INJECTION_PAYLOADS[i % SQL_INJECTION_PAYLOADS.len()];
                                SecurityTestHelper::assert_malicious_input_stored_safely(
                                    &inserted.name,
                                    payload,
                                );
...
async fn test_seaorm_input_validation_prevents_injection() -> Result<()> {
    ...
    for &payload in SQL_INJECTION_PAYLOADS {
        ...
        assert!(
            !InputValidationHelper::is_valid_url_input(payload),
        ...
        assert!(
            !InputValidationHelper::is_valid_field_name(payload),
            "Should reject malicious field name: {}",
            payload
        );

        // Test cron expression validation (before storing in SeaORM entities)
        assert!(
            !InputValidationHelper::is_valid_cron_expression(payload),
    ...
    for &payload in SQL_INJECTION_PAYLOADS.iter().take(3) {
        ...
        match filter_result {
            ...
            Err(error) => {
                ...
                assert!(
                    !error_msg.to_uppercase().contains("SQLITE"),
                    "Error should not expose database type: {}",
                    error_msg
                );
                assert!(
                    !error_msg.contains("sea_orm"),
                    "Error should not expose ORM details: {}",
                    error_msg
                );
            }
        }
    ...
    for &payload in SQL_INJECTION_PAYLOADS.iter().take(2) {
        ...
        match query_result {
            Ok(Some(model)) => {
                ...
                assert_eq!(
                    model.name, "Test Source",
                    "Mock should return controlled data"
                );
            }
            Ok(None) => {
