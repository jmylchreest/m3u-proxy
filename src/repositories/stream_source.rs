...
impl StreamSourceRepository {
    ...
    fn row_to_stream_source(&self, row: &sqlx::sqlite::SqliteRow) -> RepositoryResult<StreamSource> {
        ...
        let created_at_str: String = row.try_get("created_at")
            .map_err(|e| RepositoryError::QueryFailed {
                query: "SELECT created_at".to_string(),
                message: e.to_string(),
            })?;

        let updated_at_str: String = row.try_get("updated_at")
            .map_err(|e| RepositoryError::QueryFailed {
                query: "SELECT updated_at".to_string(),
                message: e.to_string(),
            })?;

        let created_at = DateTimeParser::parse_flexible(&created_at_str)
            .map_err(|e| RepositoryError::QueryFailed {
                query: "parse created_at".to_string(),
                message: format!("Failed to parse created_at: {}", e),
            })?;

        let updated_at = DateTimeParser::parse_flexible(&updated_at_str)
            .map_err(|e| RepositoryError::QueryFailed {
                query: "parse updated_at".to_string(),
        ...
        Ok(StreamSource {
            ...
            updated_at,
            last_ingested_at: {
                let last_ingested_str: Option<String> = row.try_get("last_ingested_at").ok();
                last_ingested_str.and_then(|s| DateTimeParser::parse_flexible(&s).ok())
            },
