...
export function useHelperAutocomplete(
  ...
  const getAutocompleteContext = useCallback((textarea: HTMLTextAreaElement): AutocompleteContext | null => {
    const { selectionStart } = textarea
  ...
  const getCompletionSuggestions = useCallback(async (
    helper: Helper, 
    query: string,
    signal?: AbortSignal
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (state.isOpen && textareaRef.current && !textareaRef.current.contains(event.target as Node)) {
