...
impl CommonValidators {
    /// Validate that a string is a valid URL with HTTP/HTTPS schemes
    pub fn http_url() -> fn(&str) -> Result<(), String> {
        |value: &str| {
    ...
    pub fn alphanumeric_with_common_symbols() -> fn(&str) -> Result<(), String> {
        |value: &str| {
            ...
            if valid {
                ...
                Err(format!(
                    "Value must contain only letters, numbers, and these symbols: {}",
                    allowed_symbols
                ))
            }
        }
    ...
    pub fn port_number() -> fn(&str) -> Result<(), String> {
        |value: &str| {
            match value.parse::<u16>() {
                Ok(port) if port > 0 => Ok(()),
                _ => Err("Port must be a number between 1 and 65535".to_string()),
            }
        }
    }
    
    /// Validate that a string is a valid non-negative integer
    pub fn non_negative_integer() -> fn(&str) -> Result<(), String> {
        |value: &str| {
            match value.parse::<u64>() {
                Ok(_) => Ok(()),
                Err(_) => Err("Value must be a non-negative integer".to_string()),
            }
        }
    }
}
...
impl ValidationUtils {
    /// Validate a single field value against multiple rules
    pub fn validate_field(
        ...
        data.insert(field_name.to_string(), value.cloned());
        
        let validator = Validator::new().rules(rules);
...
mod tests {
    ...
    fn test_validation_utils() {
        ...
        assert!(ValidationUtils::is_valid_uuid("12345678-1234-5678-9abc-123456789abc"));
        assert!(!ValidationUtils::is_valid_uuid("not-a-uuid"));
        
        assert_eq!(ValidationUtils::sanitize_string("  test  "), "test");
        
        assert_eq!(
            ValidationUtils::extract_domain("https://example.com/path"),
