...
const LIBERATION_SANS_TTF: &[u8] = include_bytes!("../assets/fonts/LiberationSansNarrow-Regular.ttf");
    ...
    fn drop(&mut self) {
        if let Some(font_path) = &self.temp_font_path {
            if let Err(e) = std::fs::remove_file(font_path) {
                warn!("Failed to clean up temporary font file {:?}: {}", font_path, e);
            } else {
...
mod tests {
    ...
    async fn test_font_manager() {
        ...
        assert!(font_path.is_some(), "Font path should be Some when embedded font is available");
        
        if let Some(path) = font_path {
            assert!(path.exists(), "Font file should exist");
            // Temporary files might not end with .ttf, just check that we have a path
            assert!(!path.to_string_lossy().is_empty(), "Font file path should not be empty");
        }
        
        // Test FFmpeg parameter generation (should now be available)
        let ffmpeg_param = font_manager.get_ffmpeg_font_param().await.unwrap();
        assert!(ffmpeg_param.is_some(), "FFmpeg param should be Some when embedded font is available");
        
        if let Some(param) = ffmpeg_param {
            assert!(param.starts_with("fontfile="), "FFmpeg param should start with fontfile=");
        }
        
        // Test font family
        assert_eq!(EmbeddedFontManager::get_font_family(), "Liberation Sans Narrow");
