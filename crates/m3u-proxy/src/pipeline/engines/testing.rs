use super::{ChannelDataMappingEngine, ProgramDataMappingEngine, DataMappingEngine, StreamRuleProcessor, EpgRuleProcessor};
use super::rule_processor::{RegexEvaluator, EpgProgram};
    ...
    pub fn create_sample_epg_programs() -> Vec<EpgProgram> {
        ...
        vec![
            ...
            EpgProgram {
                ...
                program_icon: None,
                start_time: DateTime::parse_from_rfc3339("2024-01-01T23:00:00Z").unwrap().with_timezone(&Utc),
...
mod tests {
    ...
    fn test_epg_rule_category_filtering() {
        ...
        let rule_expression = r#"program_category equals "Movies" SET program_title = "MOVIE: Action Hero""#;
        
        let result = EpgDataMappingTestService::test_single_epg_rule(
        ...
        let modified_movie = result.results.iter()
    ...
    fn test_epg_rule_channel_based_filtering() {
        ...
        for test_result in &result.results {
            if test_result.initial_program.channel_id == "channel_1" {
                ...
                assert_eq!(test_result.final_program.language, Some("en-US".to_string()));
            } else {
                assert!(!test_result.was_modified);
            }
        }
    ...
    fn test_epg_rule_title_transformation() {
        ...
        let rule_expression = r#"program_title matches "^(.+): (.+)$" SET program_title = "$2 ($1)""#;
        ...
        let modified_program = result.results.iter()
        ...
        assert_eq!(modified_program.final_program.title, "Action Hero (Movie Night)");
    }
    ...
    fn test_epg_rule_conditional_assignment() {
        ...
        let rule_expression = r#"program_category equals "Movies" SET subtitles ?= "No subtitles available""#;
        ...
        assert!(modified_program.was_modified);
        assert_eq!(modified_program.final_program.subtitles, Some("No subtitles available".to_string()));
    }
}