...
export function EnhancedThemeProvider({
  ...
  const parseThemeColors = useCallback(async (themeId: string) => {
    try {
      ...
      const lightColors = rootMatch ? extractColors(rootMatch[1]) : {
        primary: '', accent: '', background: '', secondary: ''
      }
      
      // Extract dark mode colors from .dark
      const darkMatch = cssText.match(/\.dark\s*\{([^}]+)\}/)
      const darkColors = darkMatch ? extractColors(darkMatch[1]) : {
        primary: '', accent: '', background: '', secondary: ''
      }
  ...
  useEffect(() => {
    ...
    const finalTheme = (savedTheme && themes.find(t => t.id === savedTheme)) ? savedTheme : defaultThemeProp
    ...
    const immediateActualMode = finalMode === 'system' 
