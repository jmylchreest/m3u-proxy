use crate::models::{
    ...
    data_mapping::{
        DataMappingAction, DataMappingActionType, DataMappingCondition, 
        DataMappingRule, DataMappingRuleWithDetails, MappedChannel
    },
...
impl DataMappingEngine {
    ...
    pub fn apply_mapping_rules(
        ...
        for channel in channels {
            ...
            let mapped = self.apply_rules_to_channel(channel, &active_rules, &logo_assets)?;
    ...
    fn apply_rules_to_channel(
        ...
        for rule in rules.iter() {
            if self.evaluate_rule_conditions(&mapped.original, &rule.conditions)? {
                ...
                let mutations = self.apply_rule_actions(&mut mapped, &rule.actions, logo_assets)?;
    ...
    fn evaluate_rule_conditions(
        ...
        for i in 1..conditions.len() {
            ...
            let logical_op = conditions[i].logical_operator.as_ref()
    ...
    fn evaluate_condition(
        ...
        match condition.operator {
            ...
            FilterOperator::Contains => Ok(field_value.contains(&condition.value)),
            FilterOperator::NotContains => Ok(!field_value.contains(&condition.value)),
            FilterOperator::StartsWith => Ok(field_value.starts_with(&condition.value)),
            FilterOperator::EndsWith => Ok(field_value.ends_with(&condition.value)),
            FilterOperator::Matches => {
                let regex = self.get_or_create_regex(&condition.value)?;
        ...
        for action in actions {
            ...
            match action.action_type {
                ...
                DataMappingActionType::SetLogo => {
                    if let Some(logo_id) = action.logo_asset_id {
                        if let Some(logo_asset) = logo_assets.get(&logo_id) {
                            let logo_url = format!("/api/logos/{}", logo_asset.id);
                            self.set_field_value(mapped, &action.target_field, Some(logo_url.clone()));
                            action_applied = true;
                            debug!(
                                "SetLogo: {} '{}' -> logo '{}' ({})",
                                action.target_field,
                ...
                DataMappingActionType::TransformValue => {
                    ...
                    debug!("TransformValue action not yet implemented for field {}", action.target_field);
                },
            }
    ...
    fn get_or_create_regex(&mut self, pattern: &str) -> Result<&Regex, Box<dyn std::error::Error>> {
    ...
    pub fn test_mapping_rule(
        ...
        for channel in channels {
            if self.evaluate_rule_conditions(&channel, &test_rule.conditions)? {
                ...
                self.apply_rule_actions(&mut mapped, &test_rule.actions, &logo_assets)?;
                mapped_channels.push(mapped);
            }
        }
