...
impl DatabaseOperations {
    ...
    where
        F: Fn() -> std::pin::Pin<
            Box<dyn std::future::Future<Output = AppResult<R>> + Send + 'static>,
        >,
        R: Send,
    {
        ...
        Err(last_error.unwrap_or_else(|| {
            AppError::internal(format!(
                "Database operation '{operation_name}' failed after {max_attempts} attempts"
            ))
        }))
    }
    ...
    pub async fn save_epg_programs_in_batches(
        ...
        debug!(
            "Starting batched EPG program insertion for {} items with batch size {}",
            total_items, batch_size
        );

        for (chunk_index, chunk) in programs.chunks(batch_size).enumerate() {
            debug!(
                "Processing EPG batch {}/{} ({} items)",
                chunk_index + 1,
            ...
            loop {
                match Self::insert_epg_programs_batch(chunk.to_vec(), db, batch_config).await {
                    Ok(count) => {
                        ...
                        debug!(
                            "Successfully processed EPG batch {}: {} items",
                            chunk_index + 1,
                            count
                        );
                        ...
                        if let Some(updater) = progress_updater {
                            ...
                            let save_progress =
                                (total_processed as f64 / total_items as f64) * 80.0;
                            let total_progress = 20.0 + save_progress;
                            let batch_num = chunk_index + 1;
                            let total_batches = total_items.div_ceil(batch_size);

                            let progress_message = format!(
                                "Inserting batch {batch_num}/{total_batches} ({total_processed} of {total_items} programs)"
                            );
                    ...
                    Err(e) => {
                            ...
                            && e.to_string().to_lowercase().contains("database is locked")
                        {
                            ...
                            debug!(
                                "EPG batch {} failed (attempt {}), retrying in {:?}: {}",
                                chunk_index + 1,
                                attempts.load(std::sync::atomic::Ordering::SeqCst),
                                delay,
                                e
        ...
        debug!(
            "Completed batched EPG program insertion: {} items processed successfully",
            total_processed
        );
    ...
    pub async fn insert_epg_programs_batch(
        ...
        debug!(
            "Inserting batch of {} EPG programs using multi-value INSERT",
            batch_size
        );
        ...
        for chunk in programs.chunks(max_records_per_query) {
            ...
            let result = txn.execute(stmt).await.map_err(|e| {
                AppError::internal(format!("Failed to insert EPG programs batch: {e}"))
            })?;

            total_inserted += result.rows_affected() as usize;
            trace!(
                "Inserted {} programs in multi-value query",
        ...
        debug!(
            "Successfully inserted {} EPG programs in optimized batch",
            total_inserted
        );
        Ok(total_inserted)
    }
    ...
    pub async fn insert_stream_channels_batch(
        ...
        debug!(
            "Inserting batch of {} stream channels using multi-value INSERT",
            batch_size
        );
        ...
        for chunk in channels.chunks(max_records_per_query) {
            ...
            total_inserted += result.rows_affected() as usize;
            trace!(
                "Inserted {} channels in multi-value query",
                result.rows_affected()
            );
        }
        ...
        debug!(
            "Successfully inserted {} stream channels in optimized batch",
            total_inserted
        );
        Ok(total_inserted)
    }
    ...
    pub async fn delete_epg_programs_for_source(
        ...
        loop {
            match async {
                ...
                let result = txn.execute(stmt).await.map_err(AppError::Database)?;
