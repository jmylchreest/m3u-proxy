...
use crate::services::progress_service::{OperationType, ProgressManager, ProgressService};
...
impl ProxyRegenerationService {
    ...
    pub fn new(
        ...
        let (manual_queue_sender, manual_queue_receiver) =
            mpsc::unbounded_channel::<RegenerationRequest>();
        let (auto_queue_sender, auto_queue_receiver) =
            mpsc::unbounded_channel::<RegenerationRequest>();
    ...
    pub fn shutdown(&self) {
        ...
        info!(
            "Proxy regeneration service shutdown signaled - pending operations will be cancelled"
        );
    }
    ...
    async fn process_regeneration_request(
        ...
        debug!(
            "Processing regeneration request for proxy {} (manual: {}, requested: {})",
    ...
    pub async fn queue_proxy_regeneration(
        ...
        if let Some(existing_handle) = pending.remove(&proxy_id) {
            ...
            debug!(
                "Cancelled existing regeneration timer for proxy {}",
                proxy_id
            );
        }
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                warn!(
                    "Failed to create progress manager for proxy {}: {} - attempting cleanup and retry",
                    proxy_id, e
                );
                self.progress_service
                    .force_cleanup_owner_operations(proxy_id)
                    .await;
                    ...
                    .await
                {
                    Ok(mgr) => {
                        warn!(
                            "Successfully created progress manager for proxy {} after cleanup",
                            proxy_id
                        );
                        Some(mgr)
                    }
                    Err(e2) => {
                        warn!(
                            "Failed to create progress manager for proxy {} even after cleanup: {} - skipping regeneration",
                            proxy_id, e2
                        );
                        return Ok(());
                    }
        ...
        let handle = tokio::spawn(async move {
            ...
            if let Some(ref progress_mgr) = progress_manager {
                ...
                if let Some(updater) = stage_manager.get_stage_updater("delay").await {
                    updater
                        .update_progress(
                            10.0,
                            &format!("Waiting {delay_seconds}s before regeneration"),
                        )
                        .await;
                }
            }
            debug!(
                "Proxy {} regeneration: waiting {}s before starting",
                proxy_id, delay_seconds
            );

            // Wait for the delay, but check for shutdown signal
            if service_clone
                .wait_with_cancellation(Duration::from_secs(delay_seconds))
                .await
            {
            ...
            info!(
                "Delay completed for proxy {} - queueing for sequential processing (triggered by {} source {})",
            ...
            // Check if already queued to prevent duplicates
            {
                ...
                if queued.contains(&proxy_id) {
                    debug!(
                        "Proxy {} already queued for regeneration, skipping duplicate",
                        proxy_id
                    );
                    return;
                }
            ...
            if let Err(e) = service_clone.auto_queue_sender.send(request) {
                ...
                error!(
                    "Failed to queue automatic regeneration for proxy {} after delay: {}",
                    proxy_id, e
                );
            }
    ...
    pub async fn queue_manual_regeneration(
        ...
        // Cancel any pending delayed regeneration since manual takes priority
        {
            ...
            if let Some(existing_handle) = pending.remove(&proxy_id) {
                existing_handle.abort();
                debug!(
                    "Cancelled pending auto regeneration for manual trigger: {}",
                    proxy_id
                );
            }
        }
        ...
        // Remove from auto queue if it exists there (manual takes priority)
        {
            ...
            if queued.contains(&proxy_id) {
                queued.remove(&proxy_id);
                debug!(
                    "Removed proxy {} from auto queue - manual regeneration takes priority",
                    proxy_id
                );
            }
        }
        ...
        let operation_name =
            Self::create_human_readable_manual_operation_name(&self.database, proxy_id).await;
        let progress_manager = match self
            .progress_service
            .create_staged_progress_manager(
                proxy_id,
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                warn!(
                    "Failed to create progress manager for manual proxy {}: {} - attempting cleanup and retry",
                    proxy_id, e
                );
                self.progress_service
                    .force_cleanup_owner_operations(proxy_id)
        ...
        // Check if already queued to prevent duplicates (after removing from auto queue)
        {
            ...
            if queued.contains(&proxy_id) {
                let message = format!(
                    "Proxy {proxy_id} is already queued for regeneration, rejecting duplicate manual request"
                );
                warn!("{}", message);
        ...
        debug!(
            "Queued manual regeneration for proxy {} - priority processing",
            proxy_id
        );
        Ok(())
    }
    ...
    async fn regenerate_single_proxy_internal(
        ...
        debug!(
            "Regenerating proxy {} using pipeline factory (sequential queue)",
            proxy_id
        );
        ...
        if let Some(ref _pm) = progress_manager {
            debug!(
                "Using provided ProgressManager for proxy {} regeneration",
                proxy_id
            );
        }
            ...
            Ok(result) => {
                match result.status {
                    crate::pipeline::models::PipelineStatus::Completed => {
                        ...
                        let stream_proxy_repo =
                            crate::database::repositories::StreamProxySeaOrmRepository::new(
    ...
    pub async fn find_affected_proxies(
        ...
        let proxy_ids = match source_type {
            "stream" => {
                ...
                use crate::entities::{prelude::*, proxy_sources};
                ...
                let proxy_relationships = ProxySources::find()
                    ...
                    .await
                    .map_err(|e| {
                        anyhow::anyhow!("Failed to find stream proxy relationships: {}", e)
                    })?;
            ...
            "epg" => {
                ...
                use crate::entities::{prelude::*, proxy_epg_sources};
                use sea_orm::{ColumnTrait, EntityTrait, QueryFilter};
                ...
                let proxy_relationships = ProxyEpgSources::find()
                    ...
                    .await
                    .map_err(|e| {
                        anyhow::anyhow!("Failed to find EPG proxy relationships: {}", e)
                    })?;
    ...
    async fn get_proxy_sources(
        &self,
        proxy_id: Uuid,
    ) -> Result<HashSet<(Uuid, String)>, anyhow::Error> {
        ...
        match self.find_affected_proxies(source_id, source_type).await {
            Ok(proxy_ids) => {
                if proxy_ids.is_empty() {
                    debug!(
                        "No proxies affected by {} source {} update",
                ...
                for proxy_id in proxy_ids {
                    self.schedule_proxy_regeneration(proxy_id, source_id, source_type)
                        .await;
                }
            }
            Err(e) => {
                error!(
                    "Failed to find affected proxies for {} source {}: {}",
    ...
    async fn schedule_proxy_regeneration(
        &self,
        proxy_id: Uuid,
        trigger_source_id: Uuid,
        trigger_source_type: &str,
        ...
        // Check for recent requests (within 30 seconds) to prevent rapid duplicates from scheduler
        {
            ...
            if let Some(last_request_time) = recent_requests.get(&proxy_id) {
                let seconds_since_last =
                    now.signed_duration_since(*last_request_time).num_seconds();
        ...
        self.coordinate_proxy_regeneration(proxy_id, trigger_source_id, trigger_source_type)
            .await;
    }

    /// Simple coordination: wait for all ingestions to complete, then trigger regeneration
    async fn coordinate_proxy_regeneration(
        &self,
        proxy_id: Uuid,
        completed_source_id: Uuid,
        completed_source_type: &str,
    ) {
        // Skip if already actively regenerating
        if self.has_active_regeneration(proxy_id).await {
            debug!(
                "Proxy {} already actively regenerating, skipping coordination",
                proxy_id
            );
            return;
        }
        ...
        if let Some(blocking_sources) = self.check_for_blocking_ingestions(&all_sources).await {
            debug!(
                ...
                proxy_id,
                blocking_sources.len(),
                blocking_sources
            );

            // Schedule a retry using the existing delay mechanism
            if let Err(e) = self
                .queue_proxy_regeneration_with_delay(
                    proxy_id,
                    completed_source_id,
            ...
            {
                error!(
                    "Failed to schedule delayed regeneration for proxy {}: {}",
                    proxy_id, e
                );
            }
            return;
        ...
        if let Err(e) = self
            .queue_proxy_regeneration(proxy_id, completed_source_id, completed_source_type)
            .await
        {
    ...
    async fn queue_proxy_regeneration_with_delay(
        ...
        // Cancel existing timer for this proxy if any (deduplication)
        {
            ...
            if let Some(existing_handle) = pending.remove(&proxy_id) {
                existing_handle.abort();
                debug!(
                    "Cancelled existing regeneration timer for proxy {} (coordination retry)",
                    proxy_id
                );
            }
        }
        ...
        let handle = tokio::spawn(async move {
            ...
            info!(
                "Retrying coordination for proxy {} after {}s delay",
                proxy_id, delay_seconds
            );
            ...
            let all_sources = match service_clone.get_proxy_sources(proxy_id).await {
                ...
                Err(e) => {
                    error!(
                        "Failed to get sources for delayed proxy regeneration {}: {}",
                        proxy_id, e
                    );
                    return;
                }
            ...
            if let Some(blocking_sources) = service_clone
                .check_for_blocking_ingestions(&all_sources)
                .await
            {
                debug!(
            ...
            if let Err(e) = service_clone
                .queue_proxy_regeneration(proxy_id, trigger_source_id, &trigger_source_type_owned)
                .await
            {
                error!(
                    "Failed to queue regeneration for proxy {} after delay: {}",
                    proxy_id, e
                );
            }
        });
    ...
    async fn check_for_blocking_ingestions(
        &self,
        sources: &HashSet<(Uuid, String)>,
        ...
        for (source_id, source_type) in sources {
                ...
                .await
            {
                ...
                if is_active || is_soon {
                    ...
                    if is_active {
                        debug!(
                            "Source {} ({}) is actively processing",
                            source_id, source_type
                        );
                    } else {
                        debug!(
                            "Source {} ({}) will retry within 30s",
                            source_id, source_type
                        );
                    }
                }
    ...
    pub async fn queue_affected_proxies(&self, source_id: Uuid, source_type: &str) {
        match self.find_affected_proxies(source_id, source_type).await {
            Ok(proxy_ids) => {
                ...
                for proxy_id in proxy_ids {
                    if let Err(e) = self
                        .queue_proxy_regeneration(proxy_id, source_id, source_type)
                        .await
                    {
...
mod tests {
    ...
    async fn test_deduplication() {
        ...
        let proxy_repository =
            crate::database::repositories::stream_proxy::StreamProxySeaOrmRepository::new(
                Arc::clone(&db_connection),
            );

        let http_client_factory = Arc::new(crate::utils::HttpClientFactory::new(
            None,
        ...
        let result1 = service
            .queue_proxy_regeneration(proxy_id, source_id, "stream")
            .await;
        ...
        assert!(
            result2.is_ok(),
            "Second regeneration should return Ok (deduplicated)"
        );
